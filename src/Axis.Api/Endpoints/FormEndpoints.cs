...
public static class FormEndpoints
{
    public static IEndpointRouteBuilder MapFormEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", GetForms)
            ...
            .WithName("GetForms")
            .WithSummary("List all forms for the workspace (paginated)")
        ...
        [FromQuery] int pageSize = 20)
    {
        PagedResult<FormSummaryDto> result = await mediator.Send(
            new GetFormsQuery(currentUser.WorkspaceId, page, pageSize), ct);
        ...
        CancellationToken ct)
    {
        IReadOnlyList<GetFormPickerDto> result = await mediator.Send(
            new GetFormPickerQuery(currentUser.WorkspaceId), ct);
        return Results.Ok(result);
    }
        ...
        CancellationToken ct)
    {
        FormDetailDto? result = await mediator.Send(
            new GetFormByIdQuery(formId, currentUser.WorkspaceId), ct);
    ...
    private static async Task<IResult> DeleteForm(
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(new DeleteFormCommand(formId, currentUser.WorkspaceId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(
            new RemoveFieldFromFormCommand(formId, currentUser.WorkspaceId, fieldId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
    ...
    {
        Result result = await mediator.Send(
            new ReorderFormFieldsCommand(formId, currentUser.WorkspaceId, request.FieldIds), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
}
