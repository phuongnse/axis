...
public static class FormTaskEndpoints
{
        ...
        CancellationToken ct)
    {
        IReadOnlyList<FormTaskSummaryDto> result = await mediator.Send(
            new GetMyFormTasksQuery(currentUser.UserId, currentUser.WorkspaceId, FormSubmissionStatus.Pending), ct);
        return Results.Ok(result);
    }
    ...
    {
        IReadOnlyList<FormTaskSummaryDto> result = await mediator.Send(
            new GetMyFormTasksQuery(currentUser.UserId, currentUser.WorkspaceId, FormSubmissionStatus.Submitted), ct);
        return Results.Ok(result);
    }
}
