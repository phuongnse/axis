...
public static class ExecutionEndpoints
{
    public static IEndpointRouteBuilder MapExecutionEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        executions.MapGet("/", GetAllExecutions)
            ...
            .WithSummary("List workflow executions for the team account (paginated)")
        ...
        [FromQuery] ExecutionStatus? status = null)
    {
        ...
        PagedResult<ExecutionSummaryResponse> result = await mediator.Send(
            new GetAllExecutionsQuery(currentUser.TeamAccountId, page, pageSize, status), ct);
        ...
        [FromQuery] ExecutionStatus? status = null)
    {
        ...
        PagedResult<ExecutionSummaryResponse> result = await mediator.Send(
            new GetExecutionsByWorkflowQuery(workflowId, currentUser.TeamAccountId, page, pageSize, status), ct);
        return Results.Ok(result);
    }
        ...
        CancellationToken ct)
    {
        ExecutionResponse? result = await mediator.Send(
            new GetExecutionQuery(executionId, currentUser.TeamAccountId), ct);
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(
            new CancelExecutionCommand(executionId, currentUser.TeamAccountId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        ...
        CancellationToken ct)
    {
        Result<Guid> result = await mediator.Send(new RetryExecutionCommand(
            executionId, currentUser.TeamAccountId, currentUser.UserId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.Created($"/api/executions/{result.Value}", new CreatedResponse(result.Value));
    }
        ...
        CancellationToken ct)
    {
        IReadOnlyList<ExecutionSummaryResponse> result = await mediator.Send(
            new GetRetryHistoryQuery(executionId, currentUser.TeamAccountId), ct);
        return Results.Ok(result);
    }
}
