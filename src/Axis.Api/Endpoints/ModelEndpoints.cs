...
public static class ModelEndpoints
{
    public static IEndpointRouteBuilder MapModelEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", GetModels)
            ...
            .WithName("GetModels")
            .WithSummary("List all data models for the team account")
    ...
    {
        PagedResult<ModelSummaryDto> result = await mediator.Send(new GetModelsQuery(currentUser.TeamAccountId, page, pageSize), ct);
    ...
    private static async Task<IResult> GetModel(
        ...
        CancellationToken ct)
    {
        Result<ModelDetailDto> result = await mediator.Send(new GetModelQuery(modelId, currentUser.TeamAccountId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
    ...
    private static async Task<IResult> DeleteModel(
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(new DeleteModelCommand(modelId, currentUser.TeamAccountId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
    ...
    private static async Task<IResult> RemoveField(
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(new RemoveFieldCommand(modelId, fieldId, currentUser.TeamAccountId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
    ...
    private static async Task<IResult> ReorderFields(
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(new ReorderFieldsCommand(modelId, currentUser.TeamAccountId, request.FieldIds), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
}
