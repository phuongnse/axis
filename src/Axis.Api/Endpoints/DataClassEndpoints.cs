...
public static class DataClassEndpoints
{
    public static IEndpointRouteBuilder MapDataClassEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", GetDataClasses)
            ...
            .WithSummary("List all data classes for the workspace")
        ...
        [FromQuery] int pageSize = 20)
    {
        PagedResult<DataClassSummaryDto> result = await mediator.Send(
            new GetDataClassesQuery(currentUser.WorkspaceId, page, pageSize), ct);
        ...
        CancellationToken ct)
    {
        Result<DataClassDetailDto> result = await mediator.Send(
            new GetDataClassQuery(dataClassId, currentUser.WorkspaceId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
    ...
    private static async Task<IResult> DeleteDataClass(
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(new DeleteDataClassCommand(dataClassId, currentUser.WorkspaceId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
        ...
        CancellationToken ct)
    {
        Result result = await mediator.Send(
            new RemoveFieldFromDataClassCommand(dataClassId, fieldId, currentUser.WorkspaceId), ct);
        if (result.IsFailure) return result.ToProblemDetails();
        return Results.NoContent();
    }
}
