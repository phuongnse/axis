...
{ public async Task<Result<ServiceIdentityDto>> Handle(GetServiceIdentityQuery q, CancellationToken ct) { ServiceIdentity? identity = await identities.GetAsync(q.WorkspaceId, q.ServiceIdentityId, ct); if (identity is null) return Result.Failure<ServiceIdentityDto>(ErrorCodes.NotFound, "Service identity was not found."); return await CreateServiceIdentityHandler.HasLifecycleAdministratorAuthorityAsync(memberships, q.WorkspaceId, q.ActorUserId, ct) ? Result.Success(identity.ToDto()) : CreateServiceIdentityHandler.Forbidden(); } }
