...
public sealed class UpdateTeamAccountProfileHandler(
    ITeamAccountRepository teamAccountRepo,
    ITeamAccountLogoStorageService logoStorage,
    IUnitOfWork uow)
    : ICommandHandler<UpdateTeamAccountProfileCommand>
{
    private const int MaxLogoBytes = 2_097_152; // 2 MB
    private static readonly HashSet<string> AllowedLogoTypes =
        ["image/jpeg", "image/png", "image/svg+xml"];

    public async Task<Result> Handle(UpdateTeamAccountProfileCommand command, CancellationToken cancellationToken)
    {
        ...
        if (!TeamAccountTimeZoneValidator.IsValidIanaTimeZone(command.TimeZoneId))
            return Result.Failure(ErrorCodes.BusinessRule, "Timezone must be a valid IANA timezone.");

        if (!TeamAccountLanguageValidator.IsValid(command.DefaultLanguage))
        ...
        try
        {
            teamAccount.UpdateProfile(command.Name, command.TimeZoneId, command.DefaultLanguage);
        }
