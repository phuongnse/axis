...
public sealed class RegisterTeamAccountHandler(
    ...
    ITeamAccountSlugGenerator slugGenerator,
    IEmailSender emailSender,
    IUnitOfWork uow)
    : ICommandHandler<RegisterTeamAccountCommand>
{
    ...
    [
        ...
        "team-account:settings:read", "team-account:settings:write", "team-account:delete"
    ];
    ...
    public async Task<Result> Handle(RegisterTeamAccountCommand command, CancellationToken cancellationToken)
    {
        ...
        try
        {
            Result<Email> email = Email.Create(command.TeamContactEmail);
            if (email.IsFailure)
            {
            ...
            TeamAccountSlug slug =
                await slugGenerator.GenerateUniqueSlugAsync(command.TeamAccountName, cancellationToken);
            ...
            TeamAccount teamAccount = TeamAccount.RegisterForContactVerification(
                command.TeamAccountName,
                slug,
                email.Value,
                planId,
                command.AcceptedTermsVersion,
                command.AcceptedPrivacyVersion);
            await teamAccountRepo.AddAsync(teamAccount, cancellationToken);

            Role adminRole = Role.CreateSystem("Admin", teamAccount.Id, AdminPermissions);
            Role editorRole = Role.CreateSystem("Editor", teamAccount.Id, EditorPermissions);
            Role viewerRole = Role.CreateSystem("Viewer", teamAccount.Id, ViewerPermissions);
            Role endUserRole = Role.CreateSystem("End User", teamAccount.Id, EndUserPermissions);
            ...
            await teamAccountTokenStore.CreateVerificationAsync(
                teamAccount.Id,
                tokenHash,
