...
internal sealed class S3TeamAccountLogoStorageService(IAmazonS3 s3, IConfiguration configuration)
    : ITeamAccountLogoStorageService
{
    private string BucketName => configuration["Aws:S3:TeamAccountLogoBucket"]
    ...
    {
        string key = $"team-account-logos/{teamAccountId}/{Guid.NewGuid()}";
