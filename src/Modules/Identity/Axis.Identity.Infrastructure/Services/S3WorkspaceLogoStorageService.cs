...
internal sealed class S3WorkspaceLogoStorageService(IAmazonS3 s3, IConfiguration configuration)
    : IWorkspaceLogoStorageService
{
    private string BucketName => configuration["Aws:S3:WorkspaceLogoBucket"]
    ...
    {
        string key = $"Workspace-logos/{workspaceId}/{Guid.NewGuid()}";
