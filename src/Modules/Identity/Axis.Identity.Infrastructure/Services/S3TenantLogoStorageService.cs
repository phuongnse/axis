...
internal sealed class S3TenantLogoStorageService(IAmazonS3 s3, IConfiguration configuration)
    : ITenantLogoStorageService
{
    private string BucketName => configuration["Aws:S3:TenantLogoBucket"]
    ...
    {
        string key = $"Tenant-logos/{tenantId}/{Guid.NewGuid()}";
