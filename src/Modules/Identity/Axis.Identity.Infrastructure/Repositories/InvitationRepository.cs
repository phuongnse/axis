...
internal sealed class InvitationRepository(IdentityDbContext context) : IInvitationRepository
{
    ...
    public async Task<Invitation?> GetPendingByEmailAsync(Email email, Guid teamAccountId, CancellationToken ct = default) =>
        await context.Invitations
            .FirstOrDefaultAsync(i => i.Email == email
                                   && i.TeamAccountId == teamAccountId
                                   && i.Status == InvitationStatus.Pending, ct);

    public Task<int> CountPendingAsync(Guid teamAccountId, CancellationToken ct = default) =>
        context.Invitations.CountAsync(
            i => i.TeamAccountId == teamAccountId && i.Status == InvitationStatus.Pending,
            ct);
}
