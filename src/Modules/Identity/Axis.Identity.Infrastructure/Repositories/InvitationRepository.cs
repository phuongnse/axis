...
internal sealed class InvitationRepository(IdentityDbContext context) : IInvitationRepository
{
    ...
    public async Task<Invitation?> GetPendingByEmailAsync(Email email, Guid workspaceId, CancellationToken ct = default) =>
        await context.Invitations
            .FirstOrDefaultAsync(i => i.Email == email
                                   && i.workspaceId == workspaceId
                                   && i.Status == InvitationStatus.Pending, ct);

    public Task<int> CountPendingAsync(Guid workspaceId, CancellationToken ct = default) =>
