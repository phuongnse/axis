    ...
    : ICommandHandler<DuplicateWorkflowCommand, Guid>
{
    public async Task<Result<Guid>> Handle(DuplicateWorkflowCommand command, CancellationToken cancellationToken)
    {
        ...
        string resolvedName = await ResolveUniqueCopyNameAsync(
            copy.Name, command.workspaceId, cancellationToken);
    ...
    private async Task<string> ResolveUniqueCopyNameAsync(
        string baseName, Guid workspaceId, CancellationToken cancellationToken)
    {
        if (!await workflowRepo.NameExistsAsync(baseName, workspaceId, null, cancellationToken))
            return baseName;

        for (int suffix = 2; suffix <= 50; suffix++)
        {
            string candidate = $"{baseName} ({suffix})";
            if (!await workflowRepo.NameExistsAsync(candidate, workspaceId, null, cancellationToken))
                return candidate;
        }
