...
/// <summary>Lists all workflow definitions for a workspace, ordered by last modified.</summary>
    ...
    : IQueryHandler<GetWorkflowsQuery, PagedResult<WorkflowSummaryDto>>
{
        ...
        GetWorkflowsQuery query, CancellationToken cancellationToken)
    {
        ...
        (IReadOnlyList<WorkflowDefinition> items, int totalCount) =
            await workflowRepo.GetPagedAsync(query.workspaceId, query.Page, effectivePageSize, cancellationToken);
