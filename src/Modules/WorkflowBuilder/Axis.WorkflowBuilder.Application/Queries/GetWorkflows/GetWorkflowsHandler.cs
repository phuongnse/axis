...
/// <summary>Lists all workflow definitions for a tenant, ordered by last modified.</summary>
    ...
    : IQueryHandler<GetWorkflowsQuery, PagedResult<WorkflowSummaryDto>>
{
        ...
        GetWorkflowsQuery query, CancellationToken cancellationToken)
    {
        ...
        (IReadOnlyList<WorkflowDefinition> items, int totalCount) =
            await workflowRepo.GetPagedAsync(query.tenantId, query.Page, effectivePageSize, cancellationToken);
