...
public sealed class DeactivateRuleDefinitionHandler(ICurrentUser currentUser, IRuleDefinitionRepository repository, IUnitOfWork unitOfWork)
    : ICommandHandler<DeactivateRuleDefinitionCommand, RuleDefinitionDetailDto>
{
    public async Task<Result<RuleDefinitionDetailDto>> Handle(DeactivateRuleDefinitionCommand command, CancellationToken cancellationToken)
    {
        ...
        Result deactivated = definition.Deactivate(command.ExpectedRevision, userId, DateTime.UtcNow);
