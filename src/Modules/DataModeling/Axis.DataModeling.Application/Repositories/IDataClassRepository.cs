...
public interface IDataClassRepository
{
    ...
    Task<DataClass?> GetByIdAsync(Guid id, Guid teamAccountId, CancellationToken ct = default);
    Task<IReadOnlyList<DataClass>> GetAllAsync(Guid teamAccountId, CancellationToken ct = default);
    Task<(IReadOnlyList<DataClass> Items, int TotalCount)> GetPagedAsync(Guid teamAccountId, int page, int pageSize, CancellationToken ct = default);
