    ...
    : ICommandHandler<CreateDataClassCommand, Guid>
{
    ...
    {
        if (await dataClassRepo.NameExistsAsync(command.Name, command.TeamAccountId, null, cancellationToken))
            return Result.Failure<Guid>(ErrorCodes.Conflict, $"A data class named '{command.Name}' already exists.");

        DataClass dataClass = DataClass.Create(command.Name, command.Description, command.TeamAccountId, command.CreatedBy);
