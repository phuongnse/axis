...
internal sealed class DataModelRepository(DataModelingDbContext context) : IDataModelRepository
{
    ...
    public async Task<DataModel?> GetByIdAsync(Guid id, Guid teamAccountId, CancellationToken ct = default)
        => await context.DataModels
            .FirstOrDefaultAsync(m => m.Id == id && m.TeamAccountId == teamAccountId, ct);

    public async Task<IReadOnlyList<DataModel>> GetAllAsync(Guid teamAccountId, CancellationToken ct = default)
        => await context.DataModels
    ...
    public async Task<bool> NameExistsAsync(string name, Guid teamAccountId, Guid? excludeId = null, CancellationToken ct = default)
        => await context.DataModels
            .AnyAsync(m => m.TeamAccountId == teamAccountId
