...
internal sealed class FormRepository(FormBuilderDbContext context) : IFormRepository
{
    ...
    public async Task<FormDefinition?> GetByIdAsync(Guid id, Guid organizationId, CancellationToken ct = default)
        => await context.FormDefinitions
            .FirstOrDefaultAsync(f => f.Id == id && f.OrganizationId == organizationId && !f.IsDeleted, ct);
    ...
    {
        var jsonContains = $"[{{\"type\":\"Form\",\"config\":{{\"formId\":\"{formId:D}\"}}}}]";
        var count = await context.Database
            .SqlQueryRaw<int>(
                "SELECT CAST(COUNT(*) AS int) AS \"Value\" FROM workflow_definitions WHERE steps @> {0}::jsonb",
                jsonContains)
            .FirstAsync(ct);
        return count > 0;
    }
