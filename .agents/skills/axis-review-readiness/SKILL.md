...
2. Reconcile the diff with the Design Gate, sign-off, retirement, and contract decisions. For frontend UI work, run `python scripts/axis.py check ui-foundation` and `python scripts/axis.py frontend ci`; reject a missing or contract-incompatible typed surface id, an incomplete real-symbol inventory, absent rendered owner evidence, missing contract evidence, a retained legacy composition, or an `enforced` claim without current component/browser/visual evidence. Do not accept filename or import-text scans as ownership proof.
