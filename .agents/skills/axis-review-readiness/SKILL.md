...
2. Reconcile the diff with the Design Gate, sign-off, retirement, and contract decisions. For frontend UI work, run `python scripts/axis.py check ui-foundation`; reject a prose/status claim ahead of the manifest phase, migration before `accepted`, acceptance without explicit visual evidence, or `adopted` without active-surface conformance evidence.
