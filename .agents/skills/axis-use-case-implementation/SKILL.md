...
- Non-trivial implementation **Requires** a current `$axis-review-readiness` **Ready** result followed by the configured independent reviewer before completion; primary verification is not a substitute.
...
3. Map every in-scope product and foundation AC to required AT rows and its lowest reliable automated boundary before implementation. Keep browser ATs independently runnable and outcome-focused; do not use one end-to-end test as a substitute for component, API, or persistence evidence.
...
7. Run every required AT command plus focused static checks. For the immutable implementation checkpoint, obtain or reuse `$axis-review-readiness` evidence before the configured independent review, integrate findings, and rerun affected verification before returning acceptance evidence, status, and gaps to the caller. Publication is separate and reuses review evidence for the exact checkpoint.
