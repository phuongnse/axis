...
- `read` is the default server access mode. `write` must be explicitly selected by the wrapper or environment for an approved mutation task; registration must not rely only on a runtime guard.
