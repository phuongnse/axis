...
4. Before JSX, reconcile the experience contract: user outcome; primary and recovery paths; read/edit transitions; applicable state model; information hierarchy; relationships; semantic component mapping; content/help; responsive and accessibility behavior. A reusable evaluator definition presents declared inputs, logic, and outputs; source mappings belong to application/usage views and resolved values belong to simulation or execution views. Keep composed expressions static in read-only views so partial token links do not imply inconsistent behavior; expose the canonical reference through an explicit authoring-help action. For collection CRUD, preserve the primary grid's filter, sort, page, selection, and focus through dialog operations and refresh only affected server-state projections. Decide ordinary UX details autonomously from the owner and existing patterns; stop only for missing product behavior.
