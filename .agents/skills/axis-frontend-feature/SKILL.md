...
- A cross-feature visual-system or page-archetype migration requires phase `accepted` and explicit visual-acceptance evidence returned by `$axis-ui-system`; feature work does not define or migrate an unaccepted direction.
- Phase `accepted` authorizes bounded migration, not a completion claim. Phase `adopted` requires all active supported surfaces and current conformance evidence.
...
6. Use only accepted UI contracts, except for the active golden reference owned by `$axis-ui-system`. Map product state to existing semantic roles; do not invent values or component-local conventions. This workflow **Delegates** unresolved visual deviations to `$axis-ui-system` and wire-shape deviations to `$axis-api-contract`; keep consumer classes layout-only and do not create feature-local primitives. When replacing a surface, remove obsolete controls, mappings, translations, and tests in the same slice unless the owning contract explicitly requires overlap.
