...
6. When the foundation owns a visible surface, keep implementation scope in the current Design Gate and evidence; register it at its honest lifecycle state with spec/evidence metadata in `frontend/ui-foundation.json`, bind the owner and every implementation as real symbols, and bind every active id to the contract in the typed catalog. The owner must require the compatible id and emit rendered markers. Typecheck plus rendered evidence proves ownership and coverage; version-controlled perceptual evidence is required before manifest state can become `verified` or `enforced`. The foundation contract owns behavior.
