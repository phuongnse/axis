...
- Product-neutral routing, provider, shell-state, scroll ownership, and context-transition mechanics stay here. Visible hierarchy, surface archetypes, interaction treatment, semantic shared components, and UI contract enforcement **Delegates** directly to `$axis-ui-system`; product journeys **Delegate** to `$axis-frontend-feature`.
...
5. Implement product-neutral mechanics here with focused foundation tests. Delegate only product behavior to `$axis-frontend-feature`; delegate any visible cross-feature convention or shared visual API directly to `$axis-ui-system` and consume the returned enforced-contract decision.
6. When the foundation owns a visible surface, keep implementation scope to one review unit in the current Design Gate; use `frontend/ui-coverage-profile.json` and register its honest lifecycle state, pending/accepted review state, declared evidence, and exact covered/gap/not-applicable partition in `frontend/ui-foundation.json`. Every covered requirement must trace to current acceptance ids, sidecar-declared evidence, required kinds, and modes. Bind the owner and every implementation as real symbols, and bind every active id to the contract in the typed catalog. The owner must require the compatible id and emit rendered markers. Typecheck plus rendered evidence proves ownership; no profile gaps, accepted applicable evidence, criterion-level standards assessment where claimed, and project-owner acceptance are required before manifest state can become `verified` or `enforced`. The foundation contract owns behavior.
