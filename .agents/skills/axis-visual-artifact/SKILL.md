...
description: Update Axis visual artifacts safely. Use when changing Penpot MCP design sources, design-source links, previews, low-fidelity wireframe docs, legacy Excalidraw assets, Mermaid diagrams, diagram theme configuration, or visual artifacts referenced from use-case docs.
---
...
1. Identify the artifact type.
   - Design source: Penpot file/page/frame link in a use-case `## Design Sources` row.
...
3. Edit the source of truth.
   - Edit the Penpot design source for new/updated screens, not only the committed preview.
...
4. Use Penpot MCP conservatively.
   - Start read-only: list pages/boards, inspect layer tree, and export the current target before changing it.
   - Keep writes small: one board, screen, component set, or naming cleanup per operation.
   - Keep the Penpot tab focused on the exact file/page while MCP is connected.
   - Create editable native Penpot boards, shapes, text, and components. Do not use imported SVG/bitmap groups as the source frame.
   - Use stable file, page, board, and frame names that match the owning design-source row.
   - After each write, inspect layers and export the touched board/frame; verify it is visible, editable, unclipped, and free of floating UI chrome.
   - Never paste MCP connection URLs, stream URLs, `userToken` query strings, keys, or personal export links into repo docs.
...
5. Preserve source/preview integrity.
   ...
   - Penpot docs links point to file/page/frame URLs, not MCP transport URLs.

6. Refresh derived files.
   - Penpot: export/commit previews only when the use-case `## Design Sources` table has a preview row or review needs a stable snapshot.
