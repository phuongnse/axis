...
2. Pick the narrow check.
   ...
   - Frontend: focused Vitest/Playwright or `python scripts/axis.py frontend ci` for type/lint risk. For semantic tokens, shared visual patterns, shadcn source, or provider changes, read `.agents/skills/axis-ui-system/SKILL.md` (`$axis-ui-system`) and use `python scripts/axis.py check ui-baseline` plus `python scripts/axis.py check frontend-quality`.
