...
3. Run `./scripts/check-doc-drift.sh` (bash — use Git Bash on Windows) when `src/`, `tests/`, or `docs/use-cases/` change. Use the flow-first layout in [docs/use-cases/_template/README.md](docs/use-cases/_template/README.md) and run `python3 scripts/check-use-case-docs.py --check` (also invoked by the drift script). If `docker-compose.yml` changes, update [docs/playbooks/local-dev.md](docs/playbooks/local-dev.md) in the same PR (`check-local-dev-docs.py` runs inside the drift script). CI job **Doc drift** must be green.
