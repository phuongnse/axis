...
<!-- Most important first. Tick what applies; mark N/A with a short reason. Order matches the gate/checkpoints in docs/playbooks/agent-checklist.md. -->
...
- [ ] **Checkpoint 0** — AC map complete; use-case/domain docs identified (when shipping behavior)
...
- [ ] **Gate 1** — local fast gate (`python scripts/axis.py verify`: build + vulnerability scan + format + unit test projects + frontend checks + policy tests + doc drift) ran green for paths touched; full `dotnet test Axis.sln` is enforced by CI/branch protection (N/A with reason)
- [ ] **Checkpoint 2** — owning docs updated when behavior/spec/status changed; pure refactor/style/test-only changes marked N/A
- [ ] **Checkpoint 3** — retrospective done; `REVIEW_FINDINGS.md` / patterns / use-case / `TECH_STACK.md` / `CLAUDE.md` updated if a durable rule or repeat finding emerged
