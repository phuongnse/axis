...
| Infrastructure | ✅ Done | EF Core mappings, repositories, JSONB field converters; `GetPagedAsync` with filter/sort; `BulkDeleteAsync`; `GetAllForExportAsync` (chunked streaming, 500 records per chunk). `InternalsVisibleTo("Axis.Api")` for tenant schema migrations from [E01 US-003](../E01-platform-foundation/features/F01-tenant-registration.md). |
