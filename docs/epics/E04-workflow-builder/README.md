...
| Infrastructure | ✅ Done | WorkflowBuilderDbContext, EF Core configuration (WorkflowDefinition with steps/transitions/triggers as JSONB), WorkflowRepository, 7 integration tests (Testcontainers). Schema managed via EF Core migrations (e.g. `AddUniqueConstraintOnWorkflowName`). `InternalsVisibleTo("Axis.Api")` for tenant schema migrations from [E01 US-003](../E01-platform-foundation/features/F01-tenant-registration.md). |
