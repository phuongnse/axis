...
| Infrastructure | ⚠️ Partial | `IdentityDbContext` (public schema), EF Core mappings, all repositories, `BCryptPasswordHasher` (work factor 12), `MailKitEmailSender`, `IdentityUnitOfWork`, `PasswordResetTokenStore`, `SessionStoreService` (wraps `IOpenIddictTokenManager`), `OpenIddictSeeder` (seeds `axis_spa` + `axis_m2m` clients). **Phase 2:** `IdentityGrpcService` exposes `GetUserPermissions` ([ADR-014](../../TECH_STACK.md#adr-014-grpc-for-internal-sync-rpc-and-rest-openapi-for-external-api)). |
| Contracts | ⚠️ Partial | `Axis.Identity.Contracts` — `Protos/axis/identity/v1/identity_service.proto` (`GetUserPermissions`). **Deferred (PR #93 follow-up):** Avro lifecycle events. |
...
| Frontend | ⚠️ Partial | Login (PKCE), app shell, dashboard scaffold on PR #50; register/settings and multi-tab refresh pending |
