...
| Infrastructure | ✅ Done | `IdentityDbContext` (public schema), EF Core mappings, all repositories, `BCryptPasswordHasher` (work factor 12), `MailKitEmailSender`, `IdentityUnitOfWork`, `PasswordResetTokenStore`, `SessionStoreService` (wraps `IOpenIddictTokenManager`), `OpenIddictSeeder` (seeds `axis_spa` + `axis_m2m` clients) |
...
| Frontend | ⚠️ In progress | Industrial Calm theme; `AuthCard` login/register/forgot-password; PKCE sign-in form (Zod + react-hook-form); app shell + dashboard aligned to wireframes. Gaps: silent refresh, live register/forgot APIs, full OIDC callback flow. |
