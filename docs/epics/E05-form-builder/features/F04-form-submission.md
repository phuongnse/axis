...
> Decisions: `FormSubmission.AccessToken` is a `Guid` (unique URL lookup key, not JWT); expiry is enforced via `ExpiresAt` field + `Expire()` domain method. Idempotent expiry: calling `Expire()` twice throws because status is no longer Pending.
