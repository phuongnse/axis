...
| Application | ✅ Done | `CreateFormCommand`, `DeleteFormCommand`, `UpdateFormCommand`, `GetFormsQuery`, `GetFormByIdQuery`, `AddFieldToFormCommand`, `RemoveFieldFromFormCommand`, `ReorderFormFieldsCommand`. Gap: in-memory pagination in `GetFormsHandler` (DB-level pagination deferred to Infrastructure). |
