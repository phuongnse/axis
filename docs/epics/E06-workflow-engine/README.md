...
| API | ⚠️ Partial | `ExecutionEndpoints`: list, detail, start, cancel, retry, retry-with-context, retry history. **Deferred (PR #50 follow-up):** thin-endpoint refactor (`StartExecution` input shaping in API). Form task routes live under E05 `FormTaskEndpoints` |
| Frontend | ⏳ Pending | — |
