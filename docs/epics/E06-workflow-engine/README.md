...
| Infrastructure | ⚠️ Partial | `WorkflowEngineDbContext` + all repositories + step executors registered. Schema managed via EF Core migrations (`AddExecutionSteps`, `AddWorkflowActiveStatuses`, `AddWorkflowSnapshot`). `IScriptExecutor` and `INotificationSender` are stubs (real dispatch deferred). `FormTaskSubmittedHandler` cross-module handler implemented. 27 integration tests (Testcontainers). |
