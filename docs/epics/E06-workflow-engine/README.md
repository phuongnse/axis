...
| Infrastructure | ⚠️ Partial | `WorkflowEngineDbContext` + all repositories + step executors registered. Schema managed via EF Core migrations (`AddExecutionSteps`, `AddWorkflowActiveStatuses`, `AddWorkflowSnapshot`). DbContext + UnitOfWork inlined per ADR-017. `FormTaskSubmittedHandler` cross-module handler implemented. 27 integration tests (Testcontainers). **Deferred (PR #N follow-up):** real dispatch for `IScriptExecutor` (Roslyn scripting) and `INotificationSender` (MailKit) — currently stubs. |
