...
| Tenant Provisioning (US-003) | ⚠️ Partial | Each module owns its own `OrganizationVerifiedHandler` in its Infrastructure project; the handler subscribes to Identity's `OrganizationVerifiedEvent` (Avro + Kafka, ADR-019) and runs `CREATE SCHEMA IF NOT EXISTS` + `Database.MigrateAsync()` against its own DB ([ADR-011](../../TECH_STACK.md#adr-011-per-module-database-with-schema-per-tenant-inside), [ADR-023](../../TECH_STACK.md#adr-023-per-module-ef-core-migrations-only)). Central `TenantSchemaProvisioner` + `ProvisionTenantMessage` were removed in PR #93 — extraction of a module is now a redeploy of its handler + provisioner code (no gateway refactor, per ADR-010). **Deferred (F01 US-003 follow-up):** retry/alert/polling UI, durable retry policy on the per-module handlers. |
