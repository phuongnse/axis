...
| Input/semantics | Pointer and keyboard journeys; one `main`; one localized `h1`; labels, descriptions, errors, live/status feedback, native links, form fields, autocomplete purposes, named controls, and an owner-rendered named Preferences popup with visible Language and Theme groups |
| Visual | Accepted open-Preferences light/dark × desktop/compact captures plus a VI 320 CSS-pixel text-spacing/reflow capture |
| Deterministic checks | Typed owner/state-model contract; six-consumer registry; component state evidence; open-popup naming, group, target geometry, viewport/internal/document overflow, contrast, focus, accessibility-tree, runtime-error, theme, locale, and scroll assertions; retired-popup and generic-slot sweep |

Automation is supporting evidence, not the conformance or perceptual decision. The browser contrast check enumerates visible registration and open-Preferences text in both themes and applies the WCAG relative-luminance threshold. Reflow evidence uses a 320 CSS-pixel viewport, the W3C equivalent boundary for 1280 CSS pixels at 400%, and separately applies the specified letter, word, and line spacing to both the Entry layout and the open popup. It checks document, Entry Card, and viewport-bounded popup overflow. Registration is visually representative because it exercises every owner region and the densest content; focused component and journey evidence covers the additional consumer states.

## WCAG 2.2 A/AA applicability

`Pass` means no technical failure was found within the declared page/process and current representative matrix. `N/A` means the scoped page/process contains no content or operation to which the criterion applies. These accepted findings apply to the scoped surface, not as a product-wide certification claim.
...
| 1.3.1 Info and Relationships | Pass | One main landmark, one `h1`, native forms/links, associated labels, descriptions, errors, invitation facts, status roles, and named Preferences/Language/Theme regions preserve programmatic relationships. |
| 1.3.2 Meaningful Sequence | Pass | The Preferences trigger precedes main content; the open popup presents Language then Theme without repeating the trigger label, while Card title, banner/content, and footer preserve DOM and visual order across consumers and responsive modes. |
...
| 1.4.10 Reflow | Pass | The VI 320 CSS-pixel matrix has no document, Card, or open-popup horizontal overflow; long content uses natural document scrolling and the popup is viewport bounded. |
...
| 1.4.12 Text Spacing | Pass | The VI matrix applies WCAG letter/word/line spacing to the Entry layout and open Preferences, then rechecks internal/document/popup overflow, targets, contrast, focus, and the accepted capture. |
| 1.4.13 Content on Hover or Focus | Pass | Preferences retains a localized accessible name, remains visible while used, is dismissible with Escape, and is the only information-bearing popup; no required information exists only on hover. |
...
| 2.4.6 Headings and Labels | Pass | Every state has one descriptive localized heading; fields and actions have visible or explicit accessible labels; the Preferences popup is named and its Language/Theme group labels remain visible. |
...
| 2.5.8 Target Size (Minimum) | Pass | Owner geometry asserts 44 CSS-pixel compact and 32 CSS-pixel desktop inputs/actions/consent labels and every open-Preferences option; checkbox labeling expands the activation proxy, and inline legal/footer links use the inline-target exception. |
...
| 4.1.2 Name, Role, Value | Pass | Native controls expose names and checked, invalid, required, disabled, expanded, busy, current, and described relationships; the Preferences popup carries the same localized accessible name as its trigger and its option groups expose selection; component and accessibility-tree evidence assert the contract. |
...
| Self-descriptiveness | A localized title, labels, help text, password criteria, semantic notices, pending labels, contextual footer, and visible Preferences/Language/Theme hierarchy explain the current state and available choices. |
| Conformity with expectations | Native forms, autocomplete, links, checkbox consent, explicit submit, document scrolling, and a named, dismissible Preferences popover with shared option rows follow familiar web behavior. |
...
| User engagement | Calm neutral hierarchy, one branded primary action, restrained semantic feedback, compact card width, and a bounded Preferences layer keep attention on the entry task; project-owner review accepted the captures. |

The human-centred lifecycle evidence evaluates one coherent public-entry boundary and exercises the open public Preferences layer plus adverse locale, text spacing, content height, width, theme, motion, form error, remote failure, wrong-account, pending, and recovery conditions. The technical evaluation and project-owner acceptance are complete for this replacement.
...
- `EntrySurface` owns the typed public Preferences trigger, popup accessible name, visible named option groups, responsive geometry, and shared option-row composition without duplicate decorative header chrome. Preference hooks own localized values and state transitions only; no consumer accepts a generic utility slot.
...
- The supported path contains no parallel public-entry layout, alternate Card anatomy, feature-owned `PreferencesMenu`, generic `EntrySurface` utility slot, compatibility alias, legacy wrapper, or fallback composition. The retirement sweep is `rg -n "EntrySurface|entry-surface|PreferencesMenu|utilities=|data-slot=\"entry-layout\"|data-slot=\"entry-surface\"|data-slot=\"entry-preferences\"" frontend/src frontend/tests frontend/e2e` and must be reviewed with the typed registry evidence.

## Review disposition

Technical assessment: **complete for the declared Entry Surface replacement**. Lifecycle disposition: **enforced**. Five accepted perceptual artifacts and all 18 requirement traces are current; typed consumer ownership, public-Preferences anatomy, responsive target/overflow evidence, first-line consent alignment, and retirement proof remain current.
