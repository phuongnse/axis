...
9. A record detail or form with multiple coherent information groups uses the shared tab composition: localized General first, product-owned business sections in workflow order, and optional user-relevant system information last. A single available section renders without a tablist.
10. Every expanded window keeps a stable action footer below its scrollable body. Read-only, loading, error, and unavailable states expose `Close`; editable forms expose `Cancel` plus their product-owned submit or lifecycle actions without duplicating `Close` and `Cancel` in the same footer.
11. Footer exit actions and the header close icon pass through the same owning-feature lifecycle. Dirty state requires explicit confirmation, busy state may reject closure, and accepted closure unmounts only that renderer and activates the most recent remaining expanded window.
12. Managed windows survive navigation between authenticated routes for the current app session. Sign-out, app teardown, or browser reload clears the workspace.
...
- **AC-013** The app-owned record-tab composition always owns a General section, orders product-owned business sections between General and optional system information, omits the tablist when only one section is available, keeps panels mounted so form state survives section changes, uses concise localized labels and line-tab semantics, preserves the selected section across minimize and restore, and keeps system-only data absent unless it helps the user identify, support, audit, or integrate the record.
...
| AT-007 | UI component | Shared record tabs enforce General/business/system ordering, single-section behavior, mounted form state, keyboard operation, horizontal compact overflow, and selected-section preservation across minimize/restore. | AC-004, AC-005, AC-013 | UI component test | Yes |
| AT-008 | Browser journey | Rules and Business Object dialogs consume the shared section order, expose their product-owned content in the correct section, and omit internal-only data that does not help the user. | AC-011, AC-013 | Browser automation | Yes |
...
| Record tabs | Render General first, business sections next, and optional user-relevant system information last through one app-owned line-tab composition. Keep panels mounted, selected state stable while the renderer remains mounted, labels concise and localized, and the tab row horizontally scrollable rather than wrapped on compact widths. |
