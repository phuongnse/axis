...
| [View tenant-wide execution history](tenant-execution-history/) | See all executions across all workflows so that I have a global overview of automation activity. |
...
| **Backend — medium** | ⚠️ | [workflow-history](./workflow-history/), [tenant-execution-history](./tenant-execution-history/): date/trigger filters, CSV export, role-scoped list. [cancel-execution](./cancel-execution/): abandon Wolverine jobs + cancel form tasks. **[delete-tenant](../platform-foundation/delete-tenant/):** `TenantExecutionCanceller` cancels Pending/Running executions before tenant hard-delete (`FixedTenantContext`). |
