...
1. Service operator creates a short-lived `private_key_jwt` client assertion with `iss` and `sub` equal to its client identifier, the exact token-endpoint audience, a unique `jti`, and the selected active `kid`.
...
- `iss`/`sub` mismatch, non-exact token audience, missing or replayed `jti`, stale assertion, invalid signature, or inactive Workspace grant denies and records the required redacted security outcome.
...
- **AC-002** The client assertion requires `iss = sub = client_id`, the exact token-endpoint audience, an active known `kid`, a valid ES256 signature, a bounded lifetime, and a unique unexpired `jti`.
...
- **AC-006** Each accepted `jti` is replay-protected for the assertion's valid lifetime; duplicate delivery, concurrent exchange, or retry cannot issue another token from that assertion.
