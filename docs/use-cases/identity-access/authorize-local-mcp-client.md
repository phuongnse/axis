...
2. OpenIddict validates and stores the request as a short-lived request token, exposing only its opaque OAuth `request_uri`.
...
- OpenIddict owns authorization-request persistence in its token store and exposes only the standard OAuth `request_uri`; request tokens use an absolute five-minute lifetime matching the local MCP authorization wait window.
...
>
> **Verification:** See [authorize-local-mcp-client.evidence.md](./authorize-local-mcp-client.evidence.md). Current API, frontend component, and MCP contract evidence passes; the browser/client runtime and callback-failure boundaries remain not run.
>
> **Decisions:** OpenIddict owns request validation and token-store persistence; the SPA carries only the opaque OAuth `request_uri`; MCP remains the credential-free PKCE client and does not expose account or OAuth-request tools.
