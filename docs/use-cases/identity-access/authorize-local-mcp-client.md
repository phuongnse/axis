...
6. Axis verifies that the public client identifier matches the cached request, restores and revalidates the request, then sends the authorization code and original OAuth `state` to MCP's registered loopback callback.
...
- Missing or mismatched client identifiers and invalid, expired, tampered, or replayed authorization handles fail closed at the authorization endpoint and never become an arbitrary redirect.
...
- A request-token persistence, callback, state, code, or token-exchange failure ends the authorization attempt without issuing an access token; the MCP call reports the bounded authorization failure.
...
- **AC-007** Missing or mismatched client identifiers and missing, expired, tampered, or replayed authorization handles fail closed without an open redirect, token issuance, or arbitrary external navigation.
- **AC-008** Request-token persistence or OAuth callback failures do not issue an access token and remain bounded by the existing MCP authorization timeout.
...
|---|---|
| `/sign-in?authorization_request=...&authorization_client=...` | Reuse the existing sign-in form. Keep the opaque authorization request and validated public client identifier across recoverable errors. On successful sign-in, continue the fixed authorization endpoint without rendering a separate callback screen or accepting an arbitrary return URL. |
...
>
> **Verification:** See [authorize-local-mcp-client.evidence.md](./authorize-local-mcp-client.evidence.md). API, frontend component, MCP contract, and supported app-managed client binding evidence passes; durable browser automation and callback-failure boundaries remain not run.
>
