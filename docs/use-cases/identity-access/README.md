...
| Frontend | ⚠️ Partial | Login (PKCE), app shell, and dashboard scaffold are partially implemented; settings/invitation/session flows and multi-tab refresh still pending. Organization registration screens are owned by [platform-foundation/register-org](../platform-foundation/register-org/); user registration and external-provider account setup are owned by [register-user](./register-user/). After org/user verification, the SPA runs PKCE from the callback and redirects to workspace provisioning (`completePostVerifyPkceFlow` → `/callback` → `/provisioning`). |
