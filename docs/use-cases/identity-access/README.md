...
| Frontend | ⚠️ Partial | Login (PKCE), app shell, dashboard scaffold on PR #50; settings/invitation/session flows and multi-tab refresh still pending. tenant registration use case register page is shipped under Platform Foundation docs. Register-org post-submit journey screens (email confirmation + resend, verify-email states, workspace-provisioning poll) live in `features/auth`; spec owned by [platform-foundation/register-org](../platform-foundation/register-org/). |
