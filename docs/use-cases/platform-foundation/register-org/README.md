...
> | Frontend | ⚠️ |
>
> **Gaps vs spec:** **Registration:** email/password path largely complete (`Idempotency-Key`, slug uniqueness retry). **Not yet:** Terms acceptance + slug preview on form, external-provider sign-up (ADR-027), **register-org-complete**, CAPTCHA (*Out of scope*). **Email verification (backend ✅):** tokens, resend rate limit, login blocks unverified. **Tenant provisioning (backend ✅):** event-driven coordinator, `GET /api/auth/provisioning-status?token=`. **Frontend (slice 1 — journey ✅):** `/register/confirmation`, `/auth/verify`, `/provisioning` poll UI. **Deferred (follow-up slices):** Terms/slug on register, verify PKCE session, manual retry UI, OAuth — [pr-slicing.md](../../../playbooks/pr-slicing.md).
>
> **Decisions:**
