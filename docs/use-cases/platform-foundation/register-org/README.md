...
Create a team account on the Axis platform so one user can manage multiple users, roles, invitations, and tenant-scoped resources. This is not a legal-organization registration flow; it is the transition from a solo user account to a multi-user account scope.
...
This use case is about team-account onboarding, not standalone user identity onboarding. The email collected here is a team contact email; it can be a shared mailbox such as `admin@company.com` or `it@company.com`, but it is not proof of a legal organization. Microsoft / Google / GitHub identity providers belong to user identity sign-in and provider-linking flows, not to team account registration.
...
> `POST /api/organizations` is the current internal API route; it records team account facts + legal versions and sends team contact verification. It only creates the pending team account record.
...
>
> **Decisions:**
> - `register-org` owns team account facts only: team account name, team contact email, legal acceptance, address slug, verification, and tenant provisioning.
...
> - First owner/admin identity creation is a setup-token handoff after team account verification. The dedicated team account registration page is `/register/team`; `/register/organization` remains a compatibility alias. The first-owner account handoff continues through `/register?setupToken=...`.
...
| Step | Screen | When |
|------|--------|------|
| 1 | `register-org` | Enter team account name, team contact email, address preview, and legal acceptance |
...
| Screen | When |
|--------|------|
| `register-org-states` | Validation or 5xx on the team account registration form |
...
|---|--------|------|------------|---------|
| 1 | register-org | Happy path — team account registration form | [source](./register-org.excalidraw) | [preview](./register-org.svg) |
