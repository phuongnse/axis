...
If `## Screen flow` exists, use the full table and inventory note from [register-tenant](./platform-foundation/register-tenant/README.md#wireframes). Otherwise use the minimal table below.
...
Replace path placeholders with real `[source](./…)` / `[preview](./…)` links when implementing (see [register-tenant](./platform-foundation/register-tenant/README.md#wireframes) for a filled-in table including error rows).
...
See [register-tenant § Diagrams](./platform-foundation/register-tenant/README.md#diagrams). **Related (next use case):** optional prose link after the diagrams — do not duplicate another use case’s diagram here.
