...
| AT-006 | Browser journey | Administrator manages keys and revocation with accessible pending, success, conflict, and recovery states while the collection presents canonical revision/modified-only metadata | AC-003, AC-012, AC-015 | UI component test + Browser automation | Yes |
...
|---|---|
| Service identity collection | Identify the current Workspace, service identities, non-secret key/lifecycle state, actions allowed by server-reported authority, and the canonical trailing revision/modified-only metadata group. |
