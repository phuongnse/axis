...
- **AC-011** A code-owned built-in row uses explicit `System` creation and modification actor presentation without fabricating a Human or Service subject, and uses its code-owned publication instant for both timestamps. Concurrency revision remains not applicable in the same trailing metadata group used by Workspace Rules.
...
| AT-005 | UI component | Catalog and detail use the same renderer; built-in origin only removes mutation actions, and the shared metadata contract exposes complete System provenance while keeping non-applicable concurrency revision distinct | AC-002, AC-003, AC-006, AC-011 | UI component test | Yes |
