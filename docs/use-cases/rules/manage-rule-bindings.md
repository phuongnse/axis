...
3. Rules validates workspace ownership, published-version eligibility, mapping shape and literals, target identity, and the caller's revision; when a consumer attaches the binding, Rules also validates those mappings against the consumer's transient typed context schema.
...
- **AC-003** Rules validates required mappings, optional omissions, mapping keys, literal types, cardinality, and allowed values against the referenced version; reference validation additionally rejects unknown context keys, incompatible context types/cardinality, and missing required consumer-context coverage.
...
| AT-006 | Application/Infrastructure boundaries | Business Objects persists only binding IDs and validates target, exact revision, mapping compatibility, and required field-context coverage through consumer-neutral Rules Contracts | AC-003, AC-004, AC-006 | Application test + Architecture test | Yes |
