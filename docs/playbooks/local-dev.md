...
A trusted overlay may own a finite browser verification service selected with `local-dev ... e2e --service <name> -- <test-args>`; Axis validates the service name and retains the build/run boundary without exposing an arbitrary command. The product repository owns the finite command that supplies these arguments and its deployment values; use only trusted overlay files because Compose configuration can execute containers. Host-native `dotnet run-api` is blocked while local data is bound to overlays because that path cannot prove the overlay-owned deployment values.
