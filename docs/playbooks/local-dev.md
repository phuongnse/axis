...
Commands that start the default stack or an HTTPS service validate the complete local certificate material before any Docker mutation, including CA constraints, root/leaf key identity, SAN coverage, and leaf server-auth constraints/key usage; non-HTTPS service-only commands do not require it. Strict local-dev doctor treats a non-ready certificate state as blocking, while setup may carry that repairable state only to its certificate-generation step. `local-dev certs` reuses valid material. To rotate a CA trusted through Axis, run `local-dev untrust-certs`, `local-dev certs --renew`, then `local-dev trust-certs`; the shared preflight and doctor diagnostics derive this sequence from the managed certificate state, and Axis blocks replacement while its current trust marker remains. The trust commands update the current-user Windows store (including WSL browsers) or macOS login keychain after confirming the displayed SHA-256 fingerprint. Native Linux remains manual and never invokes `sudo`: import or remove `.dev-certs/rootCA.cer` in the browser or user trust store.
