...
Pre-production: each module keeps a single **`InitialCreate`** migration generated by EF. Do **not** hand-write migration `.cs` files or patch `.Designer.cs` without re-running the tool.
...
Set the module connection string (dummy host is fine — EF only needs a reachable provider for some commands; migrate in CI/tests uses Testcontainers):
...
Regenerate from repo root (example — Identity):

```bash
# Wipe existing migrations in that module's Migrations/ folder first when resetting history.
...
Each migration must ship **both** `{timestamp}_InitialCreate.cs` and `{timestamp}_InitialCreate.Designer.cs`. Tenant modules use `*DbContextFactory` + `DesignTimePublicSchemaTenantContext` (`public` schema at design time).
