...
For first-time preparation, install the exact .NET and Node prerequisites, create and activate `.process-venv` with Python 3.14, install `requirements/process.txt` with `python -m pip install --require-hashes`, and run `processctl doctor --project-root .`. Apply `processctl setup --project-root . --profile development --apply` to restore only the consumer-owned locked project dependencies. Generate local certificates with `python scripts/axis.py local-dev certs` and install the pre-push hook with `python scripts/axis.py install-hooks`. Current-user trust remains a separate explicit `python scripts/axis.py local-dev trust-certs` action; Windows may still show its native security warning.
...
Commands that start the default stack or an HTTPS service validate the complete local certificate material before any Docker mutation, including CA constraints, root/leaf key identity, SAN coverage, and leaf server-auth constraints/key usage; non-HTTPS service-only commands do not require it. `local-dev certs` reuses valid material. To rotate a CA trusted through Axis, run `local-dev untrust-certs`, `local-dev certs --renew`, then `local-dev trust-certs`; the certificate-state diagnostics derive this sequence from the managed state, and Axis blocks replacement while its current trust marker remains. The trust commands update the current-user Windows store (including WSL browsers) or macOS login keychain after confirming the displayed SHA-256 fingerprint. Native Linux remains manual and never invokes `sudo`: import or remove `.dev-certs/rootCA.cer` in the browser or user trust store.

After the stack is ready, `python scripts/axis.py local-dev host-smoke` verifies the live web origin without overriding CA trust or accepting redirects. One platform-neutral contract selects a finite adapter for every supported host kind: Windows and WSL use a PATH-resolved Windows trust client, macOS uses the resolved curl trust configuration, and native Linux uses the active Python runtime's default SSL trust. The command proves only its named host trust boundary; a browser with a private certificate store or a process opened before trust still requires direct restart and confirmation.
...
| Shell adapters | Active shell plus `processctl doctor --project-root .` | `DOCKER_HOST` or `PATH` only when the owning native runtime resolves from another context. |
...
Prefer scoped CLI commands: `status`, `up`, `down`, `host-smoke`, `smoke`, `e2e`, and focused checks. Browser verification has one execution environment: both browser commands reconcile the mandatory stack, build the Compose E2E image, import the local CA into its container-local Chromium trust store, and keep HTTPS verification enabled. When changed source requires an image-backed runtime service rebuild, name only that service with `python scripts/axis.py local-dev e2e --build-service <compose-service> -- <playwright-args>`; repeat the option for each changed service. Runtime rebuilds are explicit and scoped, never broad or automatic. `python scripts/axis.py local-dev smoke` always runs the fixed `e2e/local-dev-smoke.pw.ts` journey. Use `python scripts/axis.py local-dev e2e -- e2e/sign-in-user.pw.ts` for acceptance evidence; add filters such as `-g "AT-001"` when one row is in scope. For an intentional visual-baseline update, persist exactly one test-owned directory with `python scripts/axis.py local-dev e2e --snapshot-output e2e/app-frame.pw.ts-snapshots -- e2e/app-frame.pw.ts --update-snapshots`; the Axis runner validates the matching test, mounts only that directory, and writes as the host user on POSIX. Review the images, then rerun the same focused test without update mode. Running `python scripts/axis.py local-dev e2e` with no args runs the full browser suite; reserve it for CI or a cross-cutting diff that invalidates every browser surface, not routine review. The reconciled stack remains running after either command. Package Playwright scripts stay behind repo wrappers. `local-dev shell [service]` is an unrestricted container diagnostic escape hatch; it is never a finite workflow or evidence command.
