...
| `TenantModuleNames` string ids | Kafka contract between modules | Add module slug when adding `TeamAccountVerifiedHandler` |
...
- [ ] If tenant provisioning: add `TeamAccountVerifiedHandler` + update `TenantModuleNames` in Identity.Contracts.
