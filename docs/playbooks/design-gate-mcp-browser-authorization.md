...
frontend/e2e/mcp-authorization.pw.ts
...
- API integration tests for interactive redirect target, opaque-only query, prompt-none behavior, resumed authorization, invalid/expired/replayed handles, and unchanged sign-in/sign-out behavior.
...
- Browser journey for MCP authorization through the real sign-in form to the loopback callback, plus the existing sign-in journey.
