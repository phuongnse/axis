...
- Portable executables still rely on publisher-documented host libraries. Before authorized downloads, setup reports every detected local-dev host blocker together and uses dynamic-loader discovery to fail early on a missing Linux ICU runtime; the installed .NET host remains authoritative for compatibility failures that cannot be proved before installation. Diagnostics print an exact host action only for a verified OS/version, never set runtime fallbacks, and never install OS packages silently.
