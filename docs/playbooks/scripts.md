...
- Use `processctl doctor --project-root .` to validate process adoption and the installed runtime; use `processctl verify --project-root . --profile <profile>` for the selected profile. `processctl setup --project-root . --profile <profile> --apply` runs only the consumer-owned project-dependency command declared in `.process/project.json`; host runtimes remain external or CI-owned prerequisites.
