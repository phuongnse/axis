...
- Use `python scripts/axis.py frontend format <source-paths>` for scoped Biome formatting and safe fixes, and `python scripts/axis.py frontend test [test-paths] [-t <name>]` for all Vitest execution and acceptance evidence; do not add arbitrary tool flags.
