...
**Reference screen:** [register-workspace](../use-cases/platform-foundation/register-workspace/README.md) (full block kit usage).

**Multi-screen journeys:** [Multi-screen journey pattern](#multi-screen-journey-pattern) below — generic checklist; use register-workspace as the worked example.
...
**Worked example:** [register-workspace](../use-cases/platform-foundation/register-workspace/README.md) (screen flow, wireframes table, diagrams). **README layout rules:** [docs-style § Use-case visual artifacts](./docs-style.md#use-case-files--wireframes--implementation-status).
...
| **Reusable blocks** | **`blocks.mjs`** ← auth / register-workspace; compose here first |
...
$env:SCREEN_FILTER = "register-workspace"
node docs/wireframes/generate-screens.mjs
docs/scripts/generate-wireframes.ps1 -Filter register-workspace
```
...
1. Add a `genXxx()` function in `generate-screens.mjs`:
   ...
   - **Auth screen with many fields / help text / provider entry points**: compose from `blocks.mjs` (`authFormField`, `paintRegisterWorkspace*`, …) — see [Agent contract](../wireframes/README.md#agent-contract)
...
6. Update the use-case `README.md`: add a `## Wireframes` row (and `## Screen flow` when >3 screens) per [docs-style § Use-case visual artifacts](./docs-style.md#use-case-files--wireframes--implementation-status) — see [register-workspace](../use-cases/platform-foundation/register-workspace/README.md) for the full layout
