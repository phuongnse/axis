...
- Consume design tokens through Tailwind classes wired in `frontend/tailwind.config.js`. Raw token values belong in `frontend/src/design-system/tokens.css`, and token registry changes belong in `frontend/src/design-system/tokens.ts`.
