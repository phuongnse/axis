...
| Module layer assembly discovery | [docs/ARCHITECTURE.md](./ARCHITECTURE.md) | Module layer project under `src/Modules/*/Axis.*.{Domain,Application,Infrastructure}` | Architecture tests | Fails when a layer project exists but architecture tests cannot load its assembly | Enforced |
