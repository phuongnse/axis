...
| Documented repo workflow bypasses Axis CLI | [scripts.md](./playbooks/scripts.md) | Command examples in repo docs and CI `run:` steps for repo-owned workflows | `python scripts/axis.py check doc-drift` rejects raw documented commands and audits CI wrapper usage | `scripts/tests/test_policy_gates.py` negative tests for raw .NET, npm, Docker, OpenSSL, grpcurl, Buf introspection, Penpot MCP, and docs script commands | **Enforced** |
