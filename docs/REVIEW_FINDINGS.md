...
| Documented repo workflow bypasses Axis CLI | [scripts.md](./playbooks/scripts.md) | Command examples in repo docs, repo skills, and CI `run:` steps for repo-owned workflows | `python scripts/axis.py check doc-drift` rejects raw documented commands, `python scripts/axis.py check codex-skills` rejects raw skill workflow commands, and the audit checks CI wrapper usage | `scripts/tests/test_policy_gates.py` negative tests for raw .NET, npm, Docker, OpenSSL, grpcurl, Buf introspection, raw design-agent package commands, docs script commands, and repo-skill command examples | **Enforced** |
...
| Use-case document contract drift | [agent-checklist.md](./playbooks/agent-checklist.md) | `docs/use-cases/**/README.md` use-case specs and domain indexes | `python scripts/axis.py check use-case-docs` parses use-case sections/tables and validates required sections, section order, table schemas, Acceptance Test Matrix AC coverage/runner values, high-level matrix cells, design-source/status table shape, Mermaid-only diagrams, self-links, and truncated domain rows | `scripts/tests/test_policy_gates.py` negative tests for matrix schema/coverage/enums, implementation-detail cells, section-order drift, design-source/status table drift, tool-specific design-source headers, and loader support for typed checker modules | **Enforced** |
| Visual artifact source/preview drift | [design-source.md](./playbooks/design-source.md) and [visual-artifact-checklist.md](./playbooks/visual-artifact-checklist.md) | Design-source rows, committed previews, and visual artifact docs/skills | `python scripts/axis.py check use-case-docs` rejects preview/export assets as editable sources and previews without same-row editable sources | `scripts/tests/test_policy_gates.py` negative tests for source-as-preview and preview-without-source; visual quality remains checklist-reviewed | **Enforced** |
...
| Codex repo skill metadata drift | [scripts.md](./playbooks/scripts.md) | Repo-scoped skills under `.agents/skills/` | `python scripts/axis.py check codex-skills` | `scripts/tests/test_policy_gates.py` rejects template TODOs, frontmatter/name mismatches, overlong or vague skill bodies, broken doc references, missing required skill chaining, raw repo workflow commands, and default prompts that do not invoke the skill | **Enforced** |
| Frontend standard controls or headless UI bypass the shadcn primitive layer | [frontend.md](./playbooks/frontend.md) | Frontend TSX outside `frontend/src/components/ui/` | `python scripts/axis.py check frontend-component-composition` rejects native `<button>`, `<input>`, `<label>`, `<select>`, `<textarea>`, and direct `@base-ui/react` imports outside the shadcn primitive layer | `scripts/tests/test_policy_gates.py` negative tests for feature-local native controls and headless UI imports | **Enforced** |
| Frontend component code bypasses design tokens for colors or shadows | [design-system.md](./playbooks/design-system.md) | Frontend TS/TSX component code outside shadcn-sourced primitives | `python scripts/axis.py check frontend-style` rejects raw neutral color utilities, raw shadow utilities, and arbitrary color/gradient values outside primitive contracts marked `source: 'shadcn'` | `scripts/tests/test_policy_gates.py` negative tests for raw neutral colors, raw shadows, arbitrary gradients, and shadcn provenance exemptions | **Enforced** |
| Shadcn UI primitive or shared component added with wrong contract/naming | [design-system.md](./playbooks/design-system.md) | `frontend/src/components/ui/*.tsx` and `frontend/src/components/shared/*` | `python scripts/axis.py check frontend-component-composition` compares shadcn UI primitive files to `frontend/src/design-system/primitive-contracts.ts`, requires `source: 'shadcn'`, verifies readiness/variant/state/accessibility/token/test metadata, and enforces `ui` kebab-case plus `shared` PascalCase/camelCase filenames | `scripts/tests/test_policy_gates.py` negative tests for missing primitive contracts, non-shadcn source metadata, missing readiness coverage, unknown token families, missing test files, and shared/ui filename drift | **Enforced** |
| Route-bound frontend surface added without consumer contract | [design-system.md](./playbooks/design-system.md) | Frontend route files importing product UI components | `python scripts/axis.py check frontend-component-composition` compares route-bound product UI files to `frontend/src/design-system/consumer-contracts.ts` and verifies owner, readiness, primitive/state/evidence metadata, and test files | `scripts/tests/test_policy_gates.py` negative tests for missing registry, missing contract row, barrel import discovery, exact route declaration matching, missing metadata, unknown evidence, and missing test files | **Enforced** |
| Case-specific automation or visual snapshot workflow proposed without a reusable contract | [scripts.md](./playbooks/scripts.md) and [design-system.md](./playbooks/design-system.md) | New or changed repo policy check, design-system automation, catalog route, visual baseline, or image-comparison workflow | Pre-code review checks the quality-bar questions before code; keep the finding human-owned unless a reusable source-of-truth contract exists | Human judgment is required to tell whether a rule is overfit; adding a meta-regex would recreate the same fragility | **Review-only** |
...
| Inline fully-qualified type names instead of `using` directives | [.editorconfig](../.editorconfig) | Style cleanup | IDE suggestions / Axis format cleanup | Low defect value; keep as style guidance unless it becomes recurring review noise | **Guidance** |
