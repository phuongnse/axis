...
- **Wolverine + RabbitMQ pairing is mature.** Saga state in Postgres + RabbitMQ for messages is the canonical Wolverine pattern; long-running orchestrators (e.g. tenant provisioning with retry/backoff/alert per [tenant provisioning use case](use-cases/platform-foundation/provision-tenant/)) fit naturally.
