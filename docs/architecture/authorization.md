...
- The reference policy proves these distinct runtime outcomes: product `Administrator` may read the published definition and list/read all product records; `Applicant` may perform the exact published-definition read action and create/read/save/submit-own-record actions; `Caseworker` may perform the exact published-definition read and list/read-all-records actions but no record-mutation action. Product authoring is outside the policy registry and uses the independent Identity-owned Workspace Product Builder decision. Lifecycle administrator bootstrap installs Solutions and assigns initial product roles; it does not make lifecycle roles product roles or Product Builder authority.
...
For Business Object runtime actions, `resourceKey` is the exact `objectKey`; a genuinely collection-wide request uses `null` and therefore requires a separate keyless grant. A missing key never matches a keyed grant or acts as a wildcard. Record `create` with `Own` means that the module stamps the authenticated subject as owner before persistence; record `list` with `Own` means the repository filters by that persisted owner before materialization. This registry owns product runtime only. Product authoring modules consume the generic Identity Product Builder decision without teaching Authorization a module catalog.

The reference policy grants product `Administrator` published-definition read plus record list/read with `All`; grants `Applicant` published-definition read plus record create/read/save/submit with `Own`; and grants `Caseworker` published-definition read plus record list/read with `All`. No other runtime action is implied.
...
- Authorization does not author policies, define a global role catalog, manage Identity lifecycle roles or Product Builder authority, create Workspaces, keep an authoring-module registry, or replace Identity authentication and Workspace access.
