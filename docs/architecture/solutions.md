...
- `SolutionInstallationOperation` is `Pending`, `Running`, `Failed` (retryable), `Blocked` (terminal validation/trust), or `Succeeded`. Confirmation moves `Pending` to `Running`; a lease holder moves `Running` to `Succeeded`, `Failed`, or `Blocked`; an authorized resume moves only `Failed` to `Pending` after fresh preflight. Lease expiry atomically returns an unfinished `Running` operation and the expired epoch's sole `Applying` step to `Pending` without changing confirmed steps. `Blocked` never resumes under the same condition.
- A step is `Pending`, `Applying`, `Confirmed`, or `Failed`. Lease holder claims `Pending` to `Applying`; matching adapter receipt/read-back confirms it; retryable failure returns it to `Pending` while recording operation `Failed`; validation, trust, or mismatch makes it `Failed` and the operation `Blocked`. Reclaiming an expired `Applying` step records its prior epoch and forces the next lease holder to perform canonical adapter read-back before any reapply: a matching hash confirms, missing content may be idempotently applied, and mismatched content blocks. Confirmed never regresses.
