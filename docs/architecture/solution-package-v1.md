...
|---|---|---|
| `authorization.policy.v1` | `policyKey` (`key`) | `{schemaVersion:1,policyKey:key,roles:[Role+],grants:[Grant+]}`. `Role` is `{key:roleKey,presentation:{languageTag:{displayName:NFC string 1..256,description?:NFC string 1..2048}}}`; presentation members sort by canonical language tag, are unique case-insensitively, and include `en`. `Grant` is `{roleKey:roleKey,actionKey:semanticPath,resourceType:semanticPath,resourceKey?:semanticPath,scope:"None"|"Own"|"All"}`. Roles sort by key; grants sort `(roleKey,actionKey,resourceType,resourceKey-or-empty,scope)`; all are unique and every roleKey exists. |
...
No component owns server-generated IDs, revisions, timestamps, actor IDs, Workspace IDs, or API-only lifecycle/action fields. Source-owned component documents contain only the stable semantic product contract and build into this v1 package; runtime projections and generated identifiers never become package identity.
