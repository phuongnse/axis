...
**Axis** is a multi-tenant SaaS low-code platform that enables Tenants to build data-driven workflow applications without writing code. Users can model their own data, design automated workflows, create interactive forms, and build custom UI pages — all within a unified visual environment.

---

## Problem Statement

Tenants of all sizes need to digitize and automate their business processes. Custom software development is expensive, slow, and requires dedicated engineering teams. Existing low-code tools are either too rigid (can't model custom data), too complex (require DevOps expertise), or too generic (lack workflow depth).
...
|---|---|---|
| **Tenant Admin** | Business owner or ops lead of a tenant | Configure the platform, manage users, build workflows |
| **Tenant Member** | Employees within the tenant | Execute workflows, manage records, fill forms |
...
- **Multi-tenancy by design** — Each Tenant is fully isolated with its own database schema.
...
| Capability | In scope |
|---|---|
| Multi-tenantanization management (registration, slug, provisioning, plans) | Yes |
