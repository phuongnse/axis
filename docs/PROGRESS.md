...
| **Phase 1 — Infrastructure foundation** | ⚠️ in progress | PR #83–#87 (Kafka, RabbitMQ, ADR-017), per-module DBs (ADR-011), Wolverine enroll (ADR-012), migrations (ADR-023), **OpenTelemetry SDK on `Axis.Api`** (ADR-018 — OTLP + Prometheus `/metrics`, optional `otel-lgtm` compose profile). Still to do: Avro + Schema Registry (ADR-019). |
