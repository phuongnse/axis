...
| **Phase 1 — Infrastructure foundation** | ⚠️ in progress | PR #83–#87 (Kafka, RabbitMQ, ADR-017). **Current:** per-module DBs + Postgres init (ADR-011), per-module Wolverine schema enrolled per DbContext (ADR-012), Identity + DataModeling migrations + all fixtures on `MigrateAsync` (ADR-023). Still to do: OpenTelemetry (ADR-018), Avro + Schema Registry (ADR-019). |
