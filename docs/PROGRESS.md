...
| **Phase 1 — Infrastructure foundation** | ⚠️ in progress | PR #83 added Kafka + Schema Registry + Vault to `docker-compose.yml`. PR #84 wired `WolverineFx.Kafka` as the events transport. PR #85 added RabbitMQ alongside as the commands/jobs/sagas transport. **This PR** narrows `Axis.Shared.Infrastructure` to abstractions only per ADR-017 — `UnitOfWork` and `AxisDbContext` base classes inlined per module. Still to do: per-module DB connection separation (ADR-011), OpenTelemetry instrumentation (ADR-018), per-module Wolverine schema (ADR-012), Avro + Schema Registry payload format (ADR-019). |
