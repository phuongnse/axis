...
- Current-state resource provenance uses the shared storage-neutral `ActorSnapshot` primitive: `User`, `ServiceIdentity`, or `System`, a stable subject identifier when one exists, and the bounded display name captured from the authenticated server principal at mutation time. Each module owns where that snapshot and its timestamps are persisted; clients never submit actor metadata, and Audit remains append-only history rather than a read-time reconstruction dependency.
