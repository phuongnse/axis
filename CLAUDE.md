...
- **Violation sweep before fix**: when a task involves fixing an architectural violation (wrong access modifier, incorrect coupling, broken pattern), always grep for all occurrences of the same violation across the **entire relevant scope** (not just the reported file) before writing any fix. Resolve all instances in the same PR. The scope is the entire layer or file type — e.g. fixing `var` in one test file means grepping all test files; fixing a missing filter in one handler means checking all handlers of the same kind. Fixing only the reported instance while leaving identical violations elsewhere creates inconsistency and false confidence in correctness.
