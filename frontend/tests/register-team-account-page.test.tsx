...
async function fillTeamAccountForm(user: ReturnType<typeof userEvent.setup>) {
...
describe('RegisterTeamAccountPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(fetch).mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.includes('/api/team-accounts/slug-preview')) {
  ...
  it('shows inline validation errors when form is empty', async () => {
    ...
    await renderWithRouter(<RegisterTeamAccountPage />, { path: '/register/team' });
  ...
  it('shows slug preview and submits the team account registration payload', async () => {
    ...
    vi.mocked(fetch).mockImplementation((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes('/api/team-accounts') && init?.method === 'POST') {
    ...
    await fillTeamAccountForm(user);
    ...
    expect(registerBody).toEqual({
      teamAccountName: "O'Brien & Co.",
      teamContactEmail: 'admin@company.com',
      ...
      if (url.includes('/api/team-accounts') && init?.method === 'POST') {
        return Promise.resolve({
          ...
          json: () =>
            Promise.resolve({
              errors: {
                teamAccountName: ['Team account name must be at least 2 characters.'],
                teamContactEmail: ['Enter a valid team contact email.'],
              },
            }),
        } as unknown as Response);
      }
