...
3. If the module provisions tenant schema: add `TenantVerifiedHandler` and update `TenantModuleNames` — see [`TenantProvisioningConventionTests`](./TenantProvisioningConventionTests.cs) and [repo-layout-discovery.md § A](../../../docs/playbooks/repo-layout-discovery.md).
