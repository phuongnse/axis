...
public class FormDefinitionTests
{
    ...
    [Fact]
    public void FormDefinition_WhenCreated_SetsNameDescriptionAndTeamAccountId()
    {
        FormDefinition form = FormDefinition.Create("Employee Intake", "New hire form", TeamAccountId, UserId);

        form.Name.Should().Be("Employee Intake");
        form.Description.Should().Be("New hire form");
        form.TeamAccountId.Should().Be(TeamAccountId);
    ...
    {
        Func<FormDefinition> act = () => FormDefinition.Create(name, null, TeamAccountId, UserId);
