...
public class RemoveFieldFromFormHandlerTests
{
    ...
    public async Task Handle_WhenFieldExists_RemovesAndSaves()
    {
        ...
        Result result = await _handler.Handle(
            new RemoveFieldFromFormCommand(form.Id, TenantId, field.Id), CancellationToken.None);
    ...
    public async Task Handle_WhenFormNotFound_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new RemoveFieldFromFormCommand(Guid.NewGuid(), TenantId, Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_WhenFormBelongsToAnotherTenant_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new RemoveFieldFromFormCommand(form.Id, otherTenantId, field.Id), CancellationToken.None);
    ...
    public async Task Handle_WhenFieldNotFound_ReturnsBusinessRule()
    {
        ...
        Result result = await _handler.Handle(
            new RemoveFieldFromFormCommand(form.Id, TenantId, Guid.NewGuid()), CancellationToken.None);
