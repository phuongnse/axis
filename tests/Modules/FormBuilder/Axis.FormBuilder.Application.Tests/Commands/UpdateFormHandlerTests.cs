...
public class UpdateFormHandlerTests
{
    ...
    {
        FormDefinition form = FormDefinition.Create("Old Name", null, TeamAccountId, "user");
        _repo.GetByIdAsync(form.Id, TeamAccountId, Arg.Any<CancellationToken>()).Returns(form);
        _repo.NameExistsAsync("New Name", TeamAccountId, form.Id, Arg.Any<CancellationToken>()).Returns(false);

        Result result = await _handler.Handle(
            new UpdateFormCommand(form.Id, TeamAccountId, "New Name", "Updated desc"), CancellationToken.None);
    ...
    public async Task Handle_WhenFormNotFound_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new UpdateFormCommand(Guid.NewGuid(), TeamAccountId, "Name", null), CancellationToken.None);
    ...
    public async Task Handle_WhenFormBelongsToAnotherTeamAccount_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new UpdateFormCommand(form.Id, otherTeamAccountId, "New Name", null), CancellationToken.None);
    ...
    public async Task Handle_WhenNameAlreadyExists_ReturnsConflict()
    {
        FormDefinition form = FormDefinition.Create("Old Name", null, TeamAccountId, "user");
        _repo.GetByIdAsync(form.Id, TeamAccountId, Arg.Any<CancellationToken>()).Returns(form);
        _repo.NameExistsAsync("Taken Name", TeamAccountId, form.Id, Arg.Any<CancellationToken>()).Returns(true);

        Result result = await _handler.Handle(
            new UpdateFormCommand(form.Id, TeamAccountId, "Taken Name", null), CancellationToken.None);
    ...
    public async Task Handle_WhenSaveThrowsUniqueConstraint_ReturnsConflict()
    {
        FormDefinition form = FormDefinition.Create("Old Name", null, TeamAccountId, "user");
        _repo.GetByIdAsync(form.Id, TeamAccountId, Arg.Any<CancellationToken>()).Returns(form);
        _repo.NameExistsAsync("Concurrent Name", TeamAccountId, form.Id, Arg.Any<CancellationToken>()).Returns(false);
        _uow.SaveChangesAsync(Arg.Any<CancellationToken>())
            .Returns<int>(_ => throw new UniqueConstraintException("unique violation"));

        Result result = await _handler.Handle(
            new UpdateFormCommand(form.Id, TeamAccountId, "Concurrent Name", null), CancellationToken.None);
