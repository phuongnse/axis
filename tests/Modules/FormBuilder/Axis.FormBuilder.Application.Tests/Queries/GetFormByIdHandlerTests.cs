...
public class GetFormByIdHandlerTests
{
    ...
    {
        FormDefinition form = FormDefinition.Create("Employee Intake", "desc", WorkspaceId, "user");
        ...
        FormDetailDto? dto = await _handler.Handle(
            new GetFormByIdQuery(form.Id, WorkspaceId), CancellationToken.None);
    ...
    public async Task Handle_WhenFormNotFound_ReturnsNull()
    {
        ...
        FormDetailDto? dto = await _handler.Handle(
            new GetFormByIdQuery(Guid.NewGuid(), WorkspaceId), CancellationToken.None);
    ...
    {
        FormDefinition form = FormDefinition.Create("Empty Form", null, WorkspaceId, "user");
        _repo.GetByIdAsync(form.Id, WorkspaceId, Arg.Any<CancellationToken>()).Returns(form);
    ...
    [Fact]
    public async Task Handle_WhenFormBelongsToAnotherWorkspace_ReturnsNull()
    {
        FormDefinition form = FormDefinition.Create("Employee Intake", null, WorkspaceId, "user");
        _repo.GetByIdAsync(form.Id, WorkspaceId, Arg.Any<CancellationToken>()).Returns(form);

        Guid otherWorkspaceId = Guid.NewGuid();
        FormDetailDto? dto = await _handler.Handle(
            new GetFormByIdQuery(form.Id, otherWorkspaceId), CancellationToken.None);
