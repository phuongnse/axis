...
public class GetFormsHandlerTests
{
    ...
    {
        FormDefinition a = FormDefinition.Create("Alpha Form", null, TeamAccountId, "user");
        FormDefinition b = FormDefinition.Create("Beta Form", "desc", TeamAccountId, "user");
        _repo.GetAllAsync(TeamAccountId, Arg.Any<CancellationToken>()).Returns([a, b]);

        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(TeamAccountId, Page: 1, PageSize: 20), CancellationToken.None);
    ...
    public async Task Handle_WhenPageSizeExceedsCap_CapsAt100()
    {
        FormDefinition[] forms = Enumerable.Range(1, 5)
            .Select(i => FormDefinition.Create($"Form {i}", null, TeamAccountId, "user"))
            .ToArray();
        _repo.GetAllAsync(TeamAccountId, Arg.Any<CancellationToken>()).Returns(forms);

        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(TeamAccountId, Page: 1, PageSize: 200), CancellationToken.None);
    ...
    public async Task Handle_WhenPageIsTwo_ReturnsSecondPage()
    {
        ...
        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(TeamAccountId, Page: 2, PageSize: 2), CancellationToken.None);
    ...
    {
        FormDefinition form = FormDefinition.Create("Form", null, TeamAccountId, "user");
        _repo.GetAllAsync(TeamAccountId, Arg.Any<CancellationToken>()).Returns([form]);

        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(TeamAccountId, Page: -5, PageSize: 10), CancellationToken.None);
    ...
    public async Task Handle_WhenPageSizeIsZeroOrNegative_ClampsToOne()
    {
        ...
        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(TeamAccountId, Page: 1, PageSize: -1), CancellationToken.None);
