...
public class GetFormsHandlerTests
{
    ...
    {
        FormDefinition a = FormDefinition.Create("Alpha Form", null, OrgId, "user");
        FormDefinition b = FormDefinition.Create("Beta Form", "desc", OrgId, "user");
        _repo.GetAllAsync(OrgId, Arg.Any<CancellationToken>()).Returns([a, b]);

        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(OrgId, Page: 1, PageSize: 20), CancellationToken.None);
    ...
    public async Task Handle_WhenPageSizeExceedsCap_CapsAt100()
    {
        ...
        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(OrgId, Page: 1, PageSize: 200), CancellationToken.None);
    ...
    public async Task Handle_WhenPageIsTwo_ReturnsSecondPage()
    {
        ...
        PagedResult<FormSummaryDto> result = await _handler.Handle(
            new GetFormsQuery(OrgId, Page: 2, PageSize: 2), CancellationToken.None);
