...
public class ExecuteNextStepHandlerTests
{
    ...
    private static WorkflowExecution CreatePendingExecution()
        => WorkflowExecution.Create(WorkflowId, WorkspaceId, TriggerType.Manual, null, new Dictionary<string, object?>());

    private static WorkflowSnapshot MakeSnapshot(Guid stepDefId, StepType stepType)
        => WorkflowSnapshot.Create(WorkflowId, WorkspaceId,
    ...
    public async Task HandleAsync_WhenNextStepIsForm_DispatchesExecuteFormStepMessage()
    {
        ...
        _workflowReader.GetSnapshotAsync(WorkflowId, WorkspaceId).Returns(MakeSnapshot(formDefId, StepType.Form));

        await CreateHandler().HandleAsync(
            new ExecuteNextStepMessage(execution.Id, WorkspaceId), CancellationToken.None);

        await _dispatcher.Received(1).PublishAsync(
            Arg.Is<ExecuteFormStepMessage>(m =>
                m.ExecutionId == execution.Id && m.workspaceId == WorkspaceId),
    ...
    public async Task HandleAsync_WhenNextStepIsHttp_DispatchesExecuteHttpStepMessage()
    {
        ...
        _execRepo.GetByIdWithStepsAsync(execution.Id, WorkspaceId).Returns(execution);
        _workflowReader.GetSnapshotAsync(WorkflowId, WorkspaceId).Returns(MakeSnapshot(httpDefId, StepType.HttpRequest));
    ...
    public async Task HandleAsync_WhenExecutionNotFound_ReturnsWithoutAction()
    {
        ...
        await CreateHandler().HandleAsync(
            new ExecuteNextStepMessage(Guid.NewGuid(), WorkspaceId), CancellationToken.None);
