...
public class ExecuteNextStepHandlerTests
{
    ...
    private static WorkflowSnapshot MakeSnapshot(Guid stepDefId, StepType stepType)
        => WorkflowSnapshot.Create(WorkflowId, TeamAccountId,
    ...
    public async Task HandleAsync_WhenNextStepIsForm_DispatchesExecuteFormStepMessage()
    {
        ...
        _workflowReader.GetSnapshotAsync(WorkflowId, TeamAccountId).Returns(MakeSnapshot(formDefId, StepType.Form));

        await CreateHandler().HandleAsync(
            new ExecuteNextStepMessage(execution.Id, TeamAccountId), CancellationToken.None);

        await _dispatcher.Received(1).PublishAsync(
            Arg.Is<ExecuteFormStepMessage>(m =>
                m.ExecutionId == execution.Id && m.TeamAccountId == TeamAccountId),
    ...
    public async Task HandleAsync_WhenNextStepIsHttp_DispatchesExecuteHttpStepMessage()
    {
        ...
        _execRepo.GetByIdWithStepsAsync(execution.Id, TeamAccountId).Returns(execution);
        _workflowReader.GetSnapshotAsync(WorkflowId, TeamAccountId).Returns(MakeSnapshot(httpDefId, StepType.HttpRequest));
    ...
    public async Task HandleAsync_WhenExecutionNotFound_ReturnsWithoutAction()
    {
        ...
        await CreateHandler().HandleAsync(
            new ExecuteNextStepMessage(Guid.NewGuid(), TeamAccountId), CancellationToken.None);
