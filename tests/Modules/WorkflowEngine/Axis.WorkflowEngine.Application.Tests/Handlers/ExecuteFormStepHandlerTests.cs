...
public class ExecuteFormStepHandlerTests
{
    ...
    public async Task HandleAsync_WithValidConfig_SetsStepWaitingAndRaisesFormStepReached()
    {
        ...
        await CreateHandler().HandleAsync(
            new ExecuteFormStepMessage(execution.Id, step.Id, TenantId, WorkflowId, ValidConfig(), execution.Context),
    ...
    public async Task HandleAsync_WithMissingFormId_FailsStepAndExecution()
    {
        ...
        await CreateHandler().HandleAsync(
            new ExecuteFormStepMessage(execution.Id, step.Id, TenantId, WorkflowId, badConfig, execution.Context),
            CancellationToken.None);
    ...
    public async Task HandleAsync_WhenExecutionNotFound_ReturnsWithoutAction()
    {
        ...
        await CreateHandler().HandleAsync(
            new ExecuteFormStepMessage(Guid.NewGuid(), Guid.NewGuid(), TenantId, WorkflowId, ValidConfig(), new Dictionary<string, object?>()),
            CancellationToken.None);
