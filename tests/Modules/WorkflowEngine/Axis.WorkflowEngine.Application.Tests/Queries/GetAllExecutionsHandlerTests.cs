...
public class GetAllExecutionsHandlerTests
{
    ...
    public async Task GetAllExecutions_WhenExecutionsExist_ReturnsPagedResult()
    {
        ...
        _execRepo.GetPagedAsync(TeamAccountId, 1, 25, null, Arg.Any<CancellationToken>())
            .Returns((summaries, 3));

        PagedResult<ExecutionSummaryResponse> result = await CreateHandler().Handle(
            new GetAllExecutionsQuery(TeamAccountId), CancellationToken.None);
    ...
    {
        _execRepo.GetPagedAsync(TeamAccountId, 1, 25, ExecutionStatus.Running, Arg.Any<CancellationToken>())
            .Returns((new List<ExecutionSummaryResponse>(), 0));

        await CreateHandler().Handle(
            new GetAllExecutionsQuery(TeamAccountId, Status: ExecutionStatus.Running),
            CancellationToken.None);

        await _execRepo.Received(1).GetPagedAsync(
            TeamAccountId, 1, 25, ExecutionStatus.Running, Arg.Any<CancellationToken>());
    }
    ...
    public async Task GetAllExecutions_WhenPageIsZeroOrNegative_ClampsToOne()
    {
        ...
        await CreateHandler().Handle(
            new GetAllExecutionsQuery(TeamAccountId, Page: -5), CancellationToken.None);

        await _execRepo.Received(1).GetPagedAsync(TeamAccountId, 1, 25, null, Arg.Any<CancellationToken>());
    }
}
