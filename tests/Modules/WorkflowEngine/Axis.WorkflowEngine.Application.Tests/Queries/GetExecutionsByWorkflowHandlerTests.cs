...
public class GetExecutionsByWorkflowHandlerTests
{
    ...
    public async Task GetExecutionsByWorkflow_WhenExecutionsExist_ReturnsPagedResult()
    {
        ...
        _execRepo.GetPagedByWorkflowAsync(WorkflowId, TeamAccountId, 1, 25, null, Arg.Any<CancellationToken>())
            .Returns((summaries, 2));

        PagedResult<ExecutionSummaryResponse> result = await CreateHandler().Handle(
            new GetExecutionsByWorkflowQuery(WorkflowId, TeamAccountId), CancellationToken.None);
    ...
    {
        _execRepo.GetPagedByWorkflowAsync(WorkflowId, TeamAccountId, 1, 25, ExecutionStatus.Failed, Arg.Any<CancellationToken>())
            .Returns((new List<ExecutionSummaryResponse>(), 0));

        await CreateHandler().Handle(
            new GetExecutionsByWorkflowQuery(WorkflowId, TeamAccountId, Status: ExecutionStatus.Failed),
            CancellationToken.None);

        await _execRepo.Received(1).GetPagedByWorkflowAsync(
            WorkflowId, TeamAccountId, 1, 25, ExecutionStatus.Failed, Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task GetExecutionsByWorkflow_WhenPageSizeExceedsMax_ClampsToHundred()
    {
        _execRepo.GetPagedByWorkflowAsync(WorkflowId, TeamAccountId, 1, 100, null, Arg.Any<CancellationToken>())
            .Returns((new List<ExecutionSummaryResponse>(), 0));

        await CreateHandler().Handle(
            new GetExecutionsByWorkflowQuery(WorkflowId, TeamAccountId, PageSize: 9999),
            CancellationToken.None);

        await _execRepo.Received(1).GetPagedByWorkflowAsync(
            WorkflowId, TeamAccountId, 1, 100, null, Arg.Any<CancellationToken>());
    }
}
