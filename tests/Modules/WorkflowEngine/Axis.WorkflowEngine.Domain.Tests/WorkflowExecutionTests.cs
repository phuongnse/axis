...
public class WorkflowExecutionTests
{
    ...
    [Fact]
    public void WorkflowExecution_WhenCreated_SetsWorkflowTenantTriggerAndPendingStatus()
    {
        WorkflowExecution exec = WorkflowExecution.Create(WorkflowId, TenantId, TriggerType.Manual, TriggeredBy, EmptyInput());

        exec.WorkflowDefinitionId.Should().Be(WorkflowId);
        exec.tenantId.Should().Be(TenantId);
    ...
    {
        WorkflowExecution original = WorkflowExecution.Create(WorkflowId, TenantId, TriggerType.Manual, TriggeredBy, EmptyInput());
        original.Start();
        original.Fail("error");
