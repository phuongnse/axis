...
public class TeamAccountRegistrationTokenStoreTests(IdentityDatabaseFixture db) : IAsyncLifetime
{
    private IdentityDbContext _ctx = null!;
    private TeamAccountRegistrationTokenStore _sut = null!;

    public Task InitializeAsync()
    {
        _ctx = db.CreateContext();
        _sut = new TeamAccountRegistrationTokenStore(_ctx);
    ...
    public async Task CreateFirstUserSetupAsync_BeforeUnitOfWorkSave_DoesNotPersistToken()
    {
        ...
        TeamAccount teamAccount = TeamAccount.RegisterForContactVerification(
            "Acme",
            TeamAccountSlug.Create($"acme-{suffix}").Value!,
        ...
        _ctx.TeamAccounts.Add(teamAccount);
        ...
        bool tokenPersistedBeforeSave = await readBeforeSave.Set<TeamAccountRegistrationToken>()
        ...
        bool tokenPersistedAfterSave = await readAfterSave.Set<TeamAccountRegistrationToken>()
            .AnyAsync(t => t.TokenHash == tokenHash);
