...
public class WorkspaceRegistrationTokenStoreTests(IdentityDatabaseFixture db) : IAsyncLifetime
{
    private IdentityDbContext _ctx = null!;
    private WorkspaceRegistrationTokenStore _sut = null!;

    public Task InitializeAsync()
    {
        _ctx = db.CreateContext();
        _sut = new WorkspaceRegistrationTokenStore(_ctx);
    ...
    public async Task CreateFirstUserSetupAsync_BeforeUnitOfWorkSave_DoesNotPersistToken()
    {
        ...
        Workspace Workspace = Workspace.RegisterTeamForContactVerification(
            "Acme",
            WorkspaceSlug.Create($"acme-{suffix}").Value!,
        ...
        bool tokenPersistedBeforeSave = await readBeforeSave.Set<WorkspaceRegistrationToken>()
        ...
        bool tokenPersistedAfterSave = await readAfterSave.Set<WorkspaceRegistrationToken>()
            .AnyAsync(t => t.TokenHash == tokenHash);
