...
public class UpdateUserProfileHandlerTests
{
    ...
    {
        User user = MakeUser();
        _userRepo.GetByIdAsync(UserId, WorkspaceId).Returns(user);

        UpdateUserProfileCommand command = new(UserId, WorkspaceId, "Bob", "Jones", null, null);
    ...
    public async Task UpdateUserProfile_WhenNoPreviousAvatar_UploadsNewAvatar()
    {
        ...
        UpdateUserProfileCommand command = new(UserId, WorkspaceId, "Alice", "Smith", avatarBytes, "image/png");
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    public async Task UpdateUserProfile_WhenAvatarExists_ReplacesOldAndDeletesIt()
    {
        ...
        byte[] avatarBytes = new byte[512];
        UpdateUserProfileCommand command = new(UserId, WorkspaceId, "Alice", "Smith", avatarBytes, "image/jpeg");
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    {
        _userRepo.GetByIdAsync(UserId, WorkspaceId).ReturnsNull();
    ...
    public async Task UpdateUserProfile_WhenFullNameTooShort_ReturnsBusinessRuleFailure()
    {
        ...
        UpdateUserProfileCommand command = new(UserId, WorkspaceId, "A", "", null, null);
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    public async Task UpdateUserProfile_WhenFullNameTooLong_ReturnsBusinessRuleFailure()
    {
        ...
        UpdateUserProfileCommand command = new(UserId, WorkspaceId, longName, "", null, null);
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    public async Task UpdateUserProfile_WhenAvatarContentTypeIsInvalid_ReturnsBusinessRuleFailure()
    {
        ...
        UpdateUserProfileCommand command = new(
            UserId, WorkspaceId, "Alice", "Smith", new byte[100], "image/gif");
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateUserProfile_WhenUserBelongsToAnotherWorkspace_ReturnsNotFound()
    {
        User user = MakeUser();
        _userRepo.GetByIdAsync(UserId, WorkspaceId).Returns(user);

        Guid otherWorkspaceId = Guid.NewGuid();
        UpdateUserProfileCommand command = new(UserId, otherWorkspaceId, "Bob", "Jones", null, null);
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
    ...
    public async Task UpdateUserProfile_WhenAvatarExceedsMaxSize_ReturnsBusinessRuleFailure()
    {
        ...
        UpdateUserProfileCommand command = new(
            UserId, WorkspaceId, "Alice", "Smith", oversized, "image/png");
        Result result = await CreateHandler().Handle(command, CancellationToken.None);
