...
public class AcceptInvitationHandlerTests
{
    ...
    private Invitation MakePendingInvitation() =>
        Invitation.Create(InvitedEmail, WorkspaceId, RoleId, InvitedById);
    ...
    public async Task AcceptInvitation_WhenTokenIsValid_CreatesUserAndAssignsRole()
    {
        ...
        _roleRepo.GetByIdsAsync(Arg.Any<IEnumerable<Guid>>(), WorkspaceId).Returns([]);
    ...
    {
        Invitation expired = InvitationTestHelper.CreateExpired(InvitedEmail, WorkspaceId, RoleId, InvitedById);
