...
public class RoleGrantPermissionsTests
{
    ...
    public void GrantMissingPermissions_WhenAdmin_AddsOnlyMissingPermissions()
    {
        ...
        bool changed = role.GrantMissingPermissions(["users:read", "team-account:settings:read"]);

        changed.Should().BeTrue();
        role.Permissions.Should().Contain("team-account:settings:read");
    ...
    public void GrantMissingPermissions_WhenNotAdmin_Throws()
    {
        ...
        Action act = () => role.GrantMissingPermissions(["team-account:settings:read"]);
