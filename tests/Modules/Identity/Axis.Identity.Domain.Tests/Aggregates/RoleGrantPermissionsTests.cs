...
public class RoleGrantPermissionsTests
{
    ...
    public void GrantMissingPermissions_WhenAdmin_AddsOnlyMissingPermissions()
    {
        ...
        bool changed = role.GrantMissingPermissions(["users:read", "workspace:settings:read"]);

        changed.Should().BeTrue();
        role.Permissions.Should().Contain("workspace:settings:read");
    ...
    public void GrantMissingPermissions_WhenNotAdmin_Throws()
    {
        ...
        Action act = () => role.GrantMissingPermissions(["workspace:settings:read"]);
