...
public class DataRecordTests
{
    ...
    [Fact]
    public void DataRecord_WhenCreated_SetsModelIdWorkspaceIdAndData()
    {
        IReadOnlyDictionary<string, object?> data = SampleData();
        DataRecord record = DataRecord.Create(ModelId, WorkspaceId, data, UserId);
    ...
    public void DataRecord_WhenCreated_SetsCreatedByAndTimestamps()
    {
        ...
        DataRecord record = DataRecord.Create(ModelId, WorkspaceId, SampleData(), UserId);
