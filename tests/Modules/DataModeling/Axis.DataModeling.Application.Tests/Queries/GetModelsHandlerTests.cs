...
public class GetModelsHandlerTests
{
    ...
    public async Task GetModels_WithExistingModels_ReturnsPagedDtos()
    {
        ...
        [
            DataModel.Create("Invoice", "Billing", null, null, TenantId, UserId),
            DataModel.Create("Contact", null, null, null, TenantId, UserId),
        ];
        _modelRepo.GetPagedAsync(TenantId, 1, 20, Arg.Any<CancellationToken>())
            .Returns((models, 2));

        PagedResult<ModelSummaryDto> result = await CreateHandler()
            .Handle(new GetModelsQuery(TenantId, 1, 20), CancellationToken.None);
    ...
    [Fact]
    public async Task GetModels_EmptyTenant_ReturnsEmptyPage()
    {
        _modelRepo.GetPagedAsync(TenantId, 1, 20, Arg.Any<CancellationToken>())
    ...
    {
        _modelRepo.GetPagedAsync(TenantId, 1, 100, Arg.Any<CancellationToken>())
            .Returns((new List<DataModel>(), 0));

        await CreateHandler().Handle(new GetModelsQuery(TenantId, 1, 999), CancellationToken.None);
