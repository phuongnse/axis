...
public class GetModelsHandlerTests
{
    ...
    public async Task GetModels_WithExistingModels_ReturnsPagedDtos()
    {
        ...
        [
            DataModel.Create("Invoice", "Billing", null, null, WorkspaceId, UserId),
            DataModel.Create("Contact", null, null, null, WorkspaceId, UserId),
        ];
        _modelRepo.GetPagedAsync(WorkspaceId, 1, 20, Arg.Any<CancellationToken>())
            .Returns((models, 2));

        PagedResult<ModelSummaryDto> result = await CreateHandler()
            .Handle(new GetModelsQuery(WorkspaceId, 1, 20), CancellationToken.None);
    ...
    [Fact]
    public async Task GetModels_EmptyWorkspace_ReturnsEmptyPage()
    {
        _modelRepo.GetPagedAsync(WorkspaceId, 1, 20, Arg.Any<CancellationToken>())
    ...
    {
        _modelRepo.GetPagedAsync(WorkspaceId, 1, 100, Arg.Any<CancellationToken>())
            .Returns((new List<DataModel>(), 0));

        await CreateHandler().Handle(new GetModelsQuery(WorkspaceId, 1, 999), CancellationToken.None);
