...
public class GetRecordsHandlerTests
{
    ...
    public async Task GetRecords_WhenModelExists_ReturnsPagedResult()
    {
        ...
        [
            DataRecord.Create(ModelId, WorkspaceId, new Dictionary<string, object?> { ["x"] = 1 }, UserId),
        ];
        _modelRepo.GetByIdAsync(ModelId, WorkspaceId).Returns(model);
        _recordRepo.GetPagedAsync(ModelId, WorkspaceId, 1, 25, null, null, null, null)
            .Returns((records.AsReadOnly() as IReadOnlyList<DataRecord>, 1));

        Result<RecordsPageDto> result = await CreateHandler().Handle(
            new GetRecordsQuery(ModelId, WorkspaceId), CancellationToken.None);
    ...
    {
        _modelRepo.GetByIdAsync(ModelId, WorkspaceId).Returns((DataModel?)null);
    ...
    public async Task GetRecords_WhenFiltersProvided_PassesFiltersToRepository()
    {
        ...
        _modelRepo.GetByIdAsync(ModelId, WorkspaceId).Returns(model);
        _recordRepo.GetPagedAsync(ModelId, WorkspaceId, 1, 25, null, filters, null, null)
            .Returns((new List<DataRecord>().AsReadOnly() as IReadOnlyList<DataRecord>, 0));

        Result<RecordsPageDto> result = await CreateHandler().Handle(
            new GetRecordsQuery(ModelId, WorkspaceId, Filters: filters), CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        await _recordRepo.Received(1).GetPagedAsync(ModelId, WorkspaceId, 1, 25, null, filters, null, null);
    }

    [Fact]
    public async Task GetRecords_WhenModelBelongsToAnotherWorkspace_ReturnsNotFound()
    {
        DataModel model = DataModel.Create("My Model", null, null, null, WorkspaceId, UserId);
        _modelRepo.GetByIdAsync(ModelId, WorkspaceId).Returns(model);
        ...
        Result<RecordsPageDto> result = await CreateHandler().Handle(
            new GetRecordsQuery(ModelId, otherWorkspaceId), CancellationToken.None);

        result.IsFailure.Should().BeTrue();
        result.ErrorCode.Should().Be(ErrorCodes.NotFound);
        await _modelRepo.Received(1).GetByIdAsync(ModelId, otherWorkspaceId);
    ...
    public async Task GetRecords_WhenSortProvided_PassesSortToRepository()
    {
        ...
        _modelRepo.GetByIdAsync(ModelId, WorkspaceId).Returns(model);
        _recordRepo.GetPagedAsync(ModelId, WorkspaceId, 1, 25, null, null, "name", "asc")
            .Returns((new List<DataRecord>().AsReadOnly() as IReadOnlyList<DataRecord>, 0));

        Result<RecordsPageDto> result = await CreateHandler().Handle(
            new GetRecordsQuery(ModelId, WorkspaceId, SortBy: "name", SortDir: "asc"), CancellationToken.None);

        result.IsSuccess.Should().BeTrue();
        await _recordRepo.Received(1).GetPagedAsync(ModelId, WorkspaceId, 1, 25, null, null, "name", "asc");
    }
}
