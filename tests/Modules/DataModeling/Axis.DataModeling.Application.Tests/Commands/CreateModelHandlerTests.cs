...
public class CreateModelHandlerTests
{
    ...
    {
        _modelRepo.NameExistsAsync("Invoice", WorkspaceId).Returns(false);

        Result<Guid> result = await CreateHandler().Handle(
            new CreateModelCommand("Invoice", "Invoicing model", null, null, WorkspaceId, UserId),
        ...
        await _modelRepo.Received(1).AddAsync(
            Arg.Is<Domain.Aggregates.DataModel>(m =>
                m.Name == "Invoice" && m.workspaceId == WorkspaceId && m.CreatedBy == UserId),
    ...
    {
        _modelRepo.NameExistsAsync("Invoice", WorkspaceId).Returns(true);

        Result<Guid> result = await CreateHandler().Handle(
            new CreateModelCommand("Invoice", null, null, null, WorkspaceId, UserId),
            CancellationToken.None);
