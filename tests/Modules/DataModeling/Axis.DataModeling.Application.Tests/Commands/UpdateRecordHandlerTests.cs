...
public class UpdateRecordHandlerTests
{
    ...
    public async Task UpdateRecord_WhenModelAndRecordExist_UpdatesDataAndSaves()
    {
        ...
        DataRecord record = DataRecord.Create(model.Id, WorkspaceId, new Dictionary<string, object?> { ["name"] = "Old" }, UserId);
        _modelRepo.GetByIdAsync(model.Id, WorkspaceId).Returns(model);
        _recordRepo.GetByIdAsync(record.Id, model.Id, WorkspaceId).Returns(record);

        Dictionary<string, object?> newData = new() { ["name"] = "New" };
        Result result = await CreateHandler().Handle(
            new UpdateRecordCommand(record.Id, model.Id, WorkspaceId, newData),
    ...
    public async Task UpdateRecord_WhenModelNotFound_ReturnsNotFound()
    {
        ...
        Result result = await CreateHandler().Handle(
            new UpdateRecordCommand(Guid.NewGuid(), Guid.NewGuid(), WorkspaceId, new Dictionary<string, object?>()),
            CancellationToken.None);
    ...
    public async Task UpdateRecord_WhenRecordNotFound_ReturnsNotFound()
    {
        DataModel model = SimpleModel();
        _modelRepo.GetByIdAsync(model.Id, WorkspaceId).Returns(model);
        _recordRepo.GetByIdAsync(Arg.Any<Guid>(), model.Id, WorkspaceId).ReturnsNull();

        Result result = await CreateHandler().Handle(
            new UpdateRecordCommand(Guid.NewGuid(), model.Id, WorkspaceId, new Dictionary<string, object?>()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task UpdateRecord_WhenModelBelongsToAnotherWorkspace_ReturnsNotFound()
    {
        DataModel model = SimpleModel();
        _modelRepo.GetByIdAsync(model.Id, WorkspaceId).Returns(model);
        ...
        Result result = await CreateHandler().Handle(
            new UpdateRecordCommand(Guid.NewGuid(), model.Id, otherWorkspaceId, new Dictionary<string, object?>()),
            CancellationToken.None);
    ...
    public async Task UpdateRecord_WhenRequiredFieldMissing_ReturnsFieldValidationError()
    {
        ...
        DataRecord record = DataRecord.Create(model.Id, WorkspaceId, new Dictionary<string, object?> { ["title"] = "Old" }, UserId);
        _modelRepo.GetByIdAsync(model.Id, WorkspaceId).Returns(model);
        _recordRepo.GetByIdAsync(record.Id, model.Id, WorkspaceId).Returns(record);

        Result result = await CreateHandler().Handle(
            new UpdateRecordCommand(record.Id, model.Id, WorkspaceId, new Dictionary<string, object?>()),
            CancellationToken.None);
