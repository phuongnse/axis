...
public class AddTransitionHandlerTests
{
    ...
    public async Task Handle_WhenStepsExistAndNoCycle_AddsTransitionAndSaves()
    {
        ...
        Result result = await _handler.Handle(
            new AddTransitionCommand(wf.Id, TenantId, start.Id, end.Id, null), CancellationToken.None);
    ...
    public async Task Handle_WhenWorkflowNotFound_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new AddTransitionCommand(Guid.NewGuid(), TenantId, Guid.NewGuid(), Guid.NewGuid(), null),
    ...
    public async Task Handle_WhenWorkflowBelongsToAnotherTenant_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(
            new AddTransitionCommand(wf.Id, otherTenantId, start.Id, end.Id, null), CancellationToken.None);
    ...
    public async Task Handle_WhenTransitionCreatesCycle_ReturnsBusinessRuleError()
    {
        ...
        Result result = await _handler.Handle(
            new AddTransitionCommand(wf.Id, TenantId, s2.Id, s1.Id, null), CancellationToken.None);
