...
public class DeleteWorkflowHandlerTests
{
    ...
    {
        WorkflowDefinition wf = WorkflowDefinition.Create("Draft Flow", null, WorkspaceId, "user");
        _repo.GetByIdAsync(wf.Id, WorkspaceId, Arg.Any<CancellationToken>()).Returns(wf);

        Result result = await _handler.Handle(new DeleteWorkflowCommand(wf.Id, WorkspaceId), CancellationToken.None);
    ...
    public async Task Handle_WhenWorkflowNotFound_ReturnsNotFound()
    {
        ...
        Result result = await _handler.Handle(new DeleteWorkflowCommand(Guid.NewGuid(), WorkspaceId), CancellationToken.None);
