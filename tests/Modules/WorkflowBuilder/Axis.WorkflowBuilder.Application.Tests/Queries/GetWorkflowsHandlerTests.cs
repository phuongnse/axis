...
public class GetWorkflowsHandlerTests
{
    ...
    public async Task GetWorkflows_WithExistingWorkflows_ReturnsPagedDtos()
    {
        ...
        [
            WorkflowDefinition.Create("Invoice Approval", null, WorkspaceId, UserId),
            WorkflowDefinition.Create("Onboarding", "New hire flow", WorkspaceId, UserId),
        ];
        _workflowRepo.GetPagedAsync(WorkspaceId, 1, 20, Arg.Any<CancellationToken>())
            .Returns((workflows, 2));

        PagedResult<WorkflowSummaryDto> result = await CreateHandler()
            .Handle(new GetWorkflowsQuery(WorkspaceId, 1, 20), CancellationToken.None);
    ...
    [Fact]
    public async Task GetWorkflows_EmptyWorkspace_ReturnsEmptyPage()
    {
        _workflowRepo.GetPagedAsync(WorkspaceId, 1, 20, Arg.Any<CancellationToken>())
    ...
    {
        _workflowRepo.GetPagedAsync(WorkspaceId, 1, 100, Arg.Any<CancellationToken>())
            .Returns((new List<WorkflowDefinition>(), 0));

        await CreateHandler().Handle(new GetWorkflowsQuery(WorkspaceId, 1, 999), CancellationToken.None);
