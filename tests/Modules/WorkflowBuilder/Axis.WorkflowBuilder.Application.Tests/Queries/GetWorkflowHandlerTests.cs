...
public class GetWorkflowHandlerTests
{
    ...
    public async Task Handle_WhenWorkflowExists_ReturnsDetailDto()
    {
        ...
        WorkflowDetailDto? dto = await _handler.Handle(new GetWorkflowQuery(wf.Id, WorkspaceId), CancellationToken.None);
    ...
    public async Task Handle_WhenWorkflowNotFound_ReturnsNull()
    {
        ...
        WorkflowDetailDto? dto = await _handler.Handle(
            new GetWorkflowQuery(Guid.NewGuid(), WorkspaceId), CancellationToken.None);
    ...
    public async Task Handle_WhenWorkflowBelongsToAnotherWorkspace_ReturnsNull()
    {
        ...
        WorkflowDetailDto? dto = await _handler.Handle(
            new GetWorkflowQuery(wf.Id, otherWorkspaceId), CancellationToken.None);
